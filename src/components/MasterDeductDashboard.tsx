...
const MasterDeductDashboard: React.FC = () => {
  ...
  const [data, setData] = useState<DeductEntry[]>([
    { id: "1", no: 1, kodePegawai: "EMP001", namaPegawai: "Ahmad Fauzi", jenisPegawai: "Tetap", periode: "2025-09", totalIncome: "Rp 6.750.000", totalDeduct: "Rp 850.000", gajiBersih: "Rp 5.900.000", keterangan: "PPH21 + Mess" },
    { id: "2", no: 2, kodePegawai: "EMP002", namaPegawai: "Siti Nurhaliza", jenisPegawai: "Kontrak", periode: "2025-09", totalIncome: "Rp 5.800.000", totalDeduct: "Rp 600.000", gajiBersih: "Rp 5.200.000", keterangan: "BPJS" },
  ]);
  ...
  const handleOpenEdit = (row: DeductEntry) => { setEditing(row); setIsModalOpen(true); };
  ...
  const filtered = data.filter(item => {
    ...
    const matchesSearch =
      ...
      item.totalDeduct.toLowerCase().includes(q);
  ...
  const handleSave = (f: PayslipFormData) => {
    if (editing) {
      ...
      const newRow: DeductEntry = { id: newId, no: 1, kodePegawai: f.kodePegawai, namaPegawai: f.namaPegawai, jenisPegawai: f.jenisPegawai, periode: f.periode, totalIncome: f.totalIncome, totalDeduct: f.totalDeduct, gajiBersih: f.gajiBersih, keterangan: f.keterangan };
  ...
  const requestDelete = (row: DeductEntry) => { setItemToDelete(row); setDeleteModalOpen(true); };
  const confirmDelete = () => {
