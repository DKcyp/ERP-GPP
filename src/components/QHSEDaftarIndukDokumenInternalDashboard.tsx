...
const sampleDokumenInternal: DokumenInternal[] = [
  ...
  },
  {
    ...
    tanggalBerlaku: "2023-01-01",
    hasilReview:
      "Dokumen sudah tidak berlaku, perlu diganti dengan format baru",
    lamaWaktuSimpan: "5 tahun",
...
const QHSEDaftarIndukDokumenInternalDashboard: React.FC = () => {
  ...
  const [selectedDokumen, setSelectedDokumen] =
    useState<DokumenInternal | null>(null);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showApproveConfirm, setShowApproveConfirm] = useState(false); // New state for approve modal
  const [showRejectConfirm, setShowRejectConfirm] = useState(false); // New state for reject modal
  const [selectedDocumentForAction, setSelectedDocumentForAction] =
    useState<DokumenInternal | null>(null); // New state for document to be approved/rejected
  ...
  const uniqueJenis = useMemo(
    () => [...new Set(dokumenData.map((item) => item.jenisDokumen))],
    [dokumenData]
  );
  const uniqueStatus = useMemo(
    () => [...new Set(dokumenData.map((item) => item.status))],
    [dokumenData]
  );
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = dokumenData.filter((item) => {
      ...
      const matchesJenis =
        filterJenis === "" || item.jenisDokumen === filterJenis;
  ...
  const handleApprove = (id: number) => {
    // Logic to update the document status to 'Aktif'
    setDokumenData((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, status: "Aktif", hasilReview: "Approved by ISO System" }
          : item
      )
    );
  ...
  const handleReject = (id: number, reason: string) => {
    // Logic to update the document status to 'Tidak Berlaku' with reason
    setDokumenData((prev) =>
      prev.map((item) =>
        item.id === id
          ? {
    ...
    if (modalMode === "add") {
      const newId =
        dokumenData.length > 0
          ? Math.max(...dokumenData.map((d) => d.id)) + 1
          : 1;
      const newNo =
        dokumenData.length > 0
          ? Math.max(...dokumenData.map((d) => d.no)) + 1
          : 1;
      ...
      setDokumenData((prev) =>
        prev.map((item) =>
          item.id === selectedDokumen.id ? { ...item, ...formData } : item
        )
      );
    }
    setShowModal(false);
    ...
    <div className="min-h-screen bg-gray-50">
                  ...
                  >
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            setSelectedDocumentForAction(dokumen);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("namaDokumen", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("nomorDokumen", e.target.value)
                      }
                      disabled={modalMode === "view"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-gray-100"
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("noRevisi", e.target.value)
                      }
                      disabled={modalMode === "view"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-gray-100"
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("tanggalBerlaku", e.target.value)
                      }
                      disabled={modalMode === "view"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-gray-100"
                      required
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleInputChange("lamaWaktuSimpan", e.target.value)
                      }
                      disabled={modalMode === "view"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-gray-100"
                      required
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        handleInputChange("hasilReview", e.target.value)
                      }
                      disabled={modalMode === "view"}
                      rows={2}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-gray-100"
      ...
      {showDeleteConfirm && selectedDokumen && (
        <ConfirmationModal
          ...
          onConfirm={() => handleDelete(selectedDokumen.id)}
          title="Hapus Dokumen"
          message={`Apakah Anda yakin ingin menghapus dokumen "${selectedDokumen.namaDokumen}"? Tindakan ini tidak dapat dibatalkan.`}
          confirmText="Hapus"
          cancelText="Batal"
        />
      )}
      ...
      {showApproveConfirm && selectedDocumentForAction && (
        <ConfirmationModal
          isOpen={showApproveConfirm}
          onClose={() => setShowApproveConfirm(false)}
          onConfirm={() => handleApprove(selectedDocumentForAction.id)}
          title="Approve Dokumen"
          message={`Apakah Anda yakin ingin menyetujui dokumen "${selectedDocumentForAction.namaDokumen}"?`}
          confirmText="Approve"
          cancelText="Batal"
      ...
      {showRejectConfirm && selectedDocumentForAction && (
        <ConfirmationModal
          isOpen={showRejectConfirm}
          onClose={() => setShowRejectConfirm(false)}
          onConfirm={(reason) =>
            selectedDocumentForAction &&
            handleReject(selectedDocumentForAction.id, reason || "")
          }
          title="Reject Dokumen"
          message={`Apakah Anda yakin ingin menolak dokumen "${selectedDocumentForAction.namaDokumen}"? Masukkan alasan penolakan.`}
          confirmText="Tolak"
          cancelText="Batal"
          isReject={true}
