...
interface IkatanDinasData {
  id: string;
  no: number;
  namaPegawai: string;
  nomorPerjanjian: string;
  tanggal: string; // YYYY-MM-DD
  periode: string; // e.g. 2025-01-01 s/d 2026-01-01
...
interface IkatanDinasForm {
  namaPegawai: string;
  nomorPerjanjian: string;
  tanggal: string;
  periode: string;
  status: IkatanDinasData["status"] | "";
  keterangan: string;
  file?: File | null;
}

const PerjanjianIkatanDinasDashboard: React.FC = () => {
  const [data, setData] = useState<IkatanDinasData[]>([
    {
      id: "1",
      no: 1,
      namaPegawai: "Satria Wibowo",
      nomorPerjanjian: "PID-001/HRD/2025",
      tanggal: "2025-09-01",
      periode: "2025-09-01 s/d 2026-09-01",
      status: "Aktif",
      keterangan: "Ikatan dinas 12 bulan",
    },
  ]);
  ...
  const [editingItem, setEditingItem] = useState<IkatanDinasData | null>(null);
  const [form, setForm] = useState<IkatanDinasForm>({ namaPegawai: "", nomorPerjanjian: "", tanggal: "", periode: "", status: "", keterangan: "", file: null });
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<IkatanDinasData | null>(null);

  useEffect(() => { const t = setTimeout(() => setAnimateRows(true), 100); return () => clearTimeout(t); }, []);

  const filtered = useMemo(() => {
    const s = search.toLowerCase();
    return data.filter((d) => d.namaPegawai.toLowerCase().includes(s) || d.nomorPerjanjian.toLowerCase().includes(s) || d.status.toLowerCase().includes(s));
  }, [data, search]);
  ...
  const resetForm = () => {
    setForm({ namaPegawai: "", nomorPerjanjian: "", tanggal: "", periode: "", status: "", keterangan: "", file: null });
    setEditingItem(null);
    setIsEditMode(false);
  };

  const openAddModal = () => { resetForm(); setIsFormOpen(true); };
  const openEditModal = (item: IkatanDinasData) => {
    setEditingItem(item);
    setForm({ namaPegawai: item.namaPegawai, nomorPerjanjian: item.nomorPerjanjian, tanggal: item.tanggal, periode: item.periode, status: item.status, keterangan: item.keterangan, file: null });
    setIsEditMode(true);
    setIsFormOpen(true);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.namaPegawai || !form.nomorPerjanjian || !form.tanggal || !form.periode || !form.status) return;
    const fileName = form.file?.name;
    const fileUrl = form.file ? URL.createObjectURL(form.file) : undefined;

    if (isEditMode && editingItem) {
      setData((prev) => prev.map((d) => d.id === editingItem.id ? { ...d, ...form, status: form.status as IkatanDinasData["status"], fileName: fileName || d.fileName, fileUrl: fileUrl || d.fileUrl } : d));
    } else {
      const nextNo = data.length > 0 ? Math.max(...data.map((x) => x.no)) + 1 : 1;
      setData((prev) => [
        { id: Date.now().toString(), no: nextNo, namaPegawai: form.namaPegawai, nomorPerjanjian: form.nomorPerjanjian, tanggal: form.tanggal, periode: form.periode, status: form.status as IkatanDinasData["status"], keterangan: form.keterangan, fileName, fileUrl },
        ...prev,
      ]);
    }
    setIsFormOpen(false);
  ...
  const confirmDelete = (item: IkatanDinasData) => { setItemToDelete(item); setDeleteModalOpen(true); };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-6">PERJANJIAN IKATAN DINAS</h1>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? "bg-white" : "bg-gray-25"} ${animateRows ? "animate-in fade-in slide-in-from-bottom-2" : "opacity-0"}`} style={{ animationDelay: animateRows ? `${index * 80}ms` : "0ms", animationFillMode: "forwards" }}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{item.nomorPerjanjian}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h3 className="text-lg font-semibold text-gray-900">{isEditMode ? "Edit Perjanjian" : "Tambah Perjanjian"}</h3>
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Nomor Perjanjian</label>
                  <input value={form.nomorPerjanjian} onChange={(e) => setForm((p) => ({ ...p, nomorPerjanjian: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-transparent" required />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tanggal</label>
                  <input type="date" value={form.tanggal} onChange={(e) => setForm((p) => ({ ...p, tanggal: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-transparent" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Periode</label>
                  <input placeholder="YYYY-MM-DD s/d YYYY-MM-DD" value={form.periode} onChange={(e) => setForm((p) => ({ ...p, periode: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-transparent" required />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
...
export default PerjanjianIkatanDinasDashboard;
