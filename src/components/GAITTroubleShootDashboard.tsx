...
import { Search, Plus, FileSpreadsheet, FileText, Clock, Edit, Trash2, Calendar, AlertTriangle } from 'lucide-react';
import TroubleShootModal, { TroubleShootForm } from './TroubleShootModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface TroubleItem extends TroubleShootForm { id: string }

const seedData = (): TroubleItem[] => [
  { id: '1', tanggal: '2025-09-01', pelapor: 'Rudi', departemen: 'Operasional', perangkat: 'LAP-OPS-01', kategori: 'Perangkat', deskripsi: 'Laptop hang saat membuka aplikasi', prioritas: 'High', status: 'Open', penangan: 'Andi', solusi: '' },
  { id: '2', tanggal: '2025-09-03', pelapor: 'Sari', departemen: 'HRD', perangkat: 'Printer-2F', kategori: 'Perangkat', deskripsi: 'Printer kertas macet', prioritas: 'Medium', status: 'Resolved', penangan: 'Budi', solusi: 'Bersihkan roller' },
  { id: '3', tanggal: '2025-09-05', pelapor: 'Budi', departemen: 'Finance', perangkat: 'LAP-FIN-03', kategori: 'Jaringan', deskripsi: 'Tidak bisa akses internet', prioritas: 'Critical', status: 'In Progress', penangan: 'Cici', solusi: '' },
];
...
const GAITTroubleShootDashboard: React.FC = () => {
  // Filters
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d => {
      const matchText = q ? `${d.pelapor} ${d.departemen} ${d.perangkat} ${d.deskripsi}`.toLowerCase().includes(q) : true;
      const matchStatus = status ? d.status === (status as TroubleItem['status']) : true;
      ...
      return matchText && matchStatus && matchPri && matchKat && matchDate;
    });
  }, [data, search, status, prioritas, kategori, dateFrom, dateTo]);
  ...
  const handleSave = (form: TroubleShootForm) => {
    if (editingItem) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">TROUBLE SHOOT</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Trouble Shoot</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6 mb-6">
            ...
            <div className="space-y-2 lg:col-span-2">
              <label className="block text-xs font-medium text-gray-700">Cari (Pelapor/Departemen/Perangkat/Keluhan)</label>
              <div className="relative">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Contoh: Rudi / Jaringan / Printer" />
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'} ${animateRows ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : '0ms', animationFillMode: 'forwards' }}>
                    ...
                    <td className="px-2 py-1 text-gray-700">{item.perangkat}</td>
                    ...
                    <td className="px-2 py-1 text-gray-700">{item.deskripsi}</td>
      ...
      <TroubleShootModal
        ...
        initialData={editingItem ? {
          ...
          perangkat: editingItem.perangkat,
        ...
        title={editingItem ? 'Edit Ticket Trouble Shoot' : 'Tambah Ticket Trouble Shoot'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.pelapor ?? ''} - ${deleteTarget?.perangkat ?? ''}`}
      />
    </div>
  );
};

export default GAITTroubleShootDashboard;
