...
const CalendarAbsensiDashboard: React.FC = () => {
  ...
  const [cutiBersama, setCutiBersama] = useState<CutiBersamaEntry[]>([
    {
      id: "1",
      no: 1,
      tanggal: `${new Date().getFullYear()}-01-02`,
  ...
  const [jadwal, setJadwal] = useState<JadwalTahunanEntry[]>([
    {
      id: "1",
      no: 1,
      tanggalMulai: `${new Date().getFullYear()}-03-10`,
      tanggalSelesai: `${new Date().getFullYear()}-03-12`,
      judul: "Raker SaVa Group",
      keterangan: "Rapat kerja tahunan",
    },
  ]);
  ...
  const handleAdjCuti = async () => {
    ...
    setCutiBersama((prev) =>
      prev.map((x) =>
        x.tanggal === adjTanggal
          ? { ...x, keterangan: adjKet || x.keterangan }
          : x
      )
    );
    setAdjTanggal("");
    ...
    setJadwal((prev) => [
      {
        id: (prev.length + 1).toString(),
        no: 1,
        tanggalMulai: jdMulai,
        tanggalSelesai: jdSelesai,
        judul: jdJudul,
        keterangan: jdKet || "-",
      },
      ...prev.map((x) => ({ ...x, no: x.no + 1 })),
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                type="date"
                value={cbTanggal}
                onChange={(e) => setCbTanggal(e.target.value)}
            ...
            <div className="md:col-span-2">
              ...
              <input
                type="text"
                value={cbKet}
                onChange={(e) => setCbKet(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  ...
                  <tr key={x.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm text-gray-900">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
                            setAdjTanggal(x.tanggal);
                            setAdjKet(x.keterangan);
                          }}
                        ...
                        <button
                          onClick={() =>
                            requestDelete("cuti", x.id, x.keterangan)
                          }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <div>
              ...
              <input
                type="date"
                value={jdMulai}
                onChange={(e) => setJdMulai(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            <div>
              ...
              <input
                type="date"
                value={jdSelesai}
                onChange={(e) => setJdSelesai(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={jdJudul}
                onChange={(e) => setJdJudul(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="Contoh: Raker Tahunan"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <input
                type="text"
                value={jdKet}
                onChange={(e) => setJdKet(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  ...
                  <tr key={x.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm text-gray-900">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
                            setJdMulai(x.tanggalMulai);
                            setJdSelesai(x.tanggalSelesai);
                            setJdJudul(x.judul);
                            setJdKet(x.keterangan || "");
                          }}
                          className="p-2 rounded-full bg-yellow-100 text-yellow-600 hover:bg-yellow-200"
                          title="Edit"
                        >
                        ...
                        <button
                          onClick={() => requestDelete("jadwal", x.id, x.judul)}
                          className="p-2 rounded-full bg-red-100 text-red-600 hover:bg-red-200"
                          title="Delete"
                        >
                          <Trash2 className="h-4 w-4" />
