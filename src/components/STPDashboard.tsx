...
import EntrySTPModal from "./EntrySTPModal";
import ConfirmDeleteModal from "./ConfirmDeleteModal";

const dummyData: STPData[] = [
  {
    id: 1,
    namaPihak: "PT Garuda Sentosa",
    npwp: "88.777.666.5-444.000",
    tanggal: "2024-09-15",
    stp: 12000000,
  },
];

const STPDashboard: React.FC = () => {
  const [data, setData] = useState<STPData[]>(dummyData);
  const [searchTerm, setSearchTerm] = useState("");

  const [isAddEditModalOpen, setIsAddEditModalOpen] = useState(false);
  const [itemToEdit, setItemToEdit] = useState<STPData | null>(null);
  ...
  const filteredData = data.filter(
    (item) =>
      item.namaPihak.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEditClick = (item: STPData) => {
    setItemToEdit(item);
    setIsAddEditModalOpen(true);
  };
  const handleSaveItem = (newItem: STPData) => {
    if (newItem.id) {
  ...
  const handleConfirmDelete = () => {
    if (itemToDeleteId)
      setData((prev) => prev.filter((i) => i.id !== itemToDeleteId));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <nav className="text-sm text-gray-600">
                ...
                <span className="text-blue-600 font-medium">STP</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-8 flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-4">
          <div className="flex items-center space-x-2 w-full md:w-auto">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Cari pihak atau NPWP..."
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                {filteredData.length === 0 && (
                  <tr>
                    ...
                    >
                      Tidak ada data STP yang ditemukan.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <ConfirmDeleteModal
        ...
        itemName={
          itemToDeleteId
            ? data.find((i) => i.id === itemToDeleteId)?.namaPihak
            : ""
        }
      />
    </div>
