...
import { Clock, Search, Download, PlusCircle, Pencil, Trash2, X } from "lucide-react";
...
type Item = {
  ...
  nama: string;
  noDosimeter: string;
  masaBerlaku: string; // ISO date string YYYY-MM-DD
};

const initialData: Item[] = [
  { id: "D-1", nama: "Andi Wijaya", noDosimeter: "DS-001", masaBerlaku: "2025-01-20" },
  { id: "D-2", nama: "Budi Santoso", noDosimeter: "DS-045", masaBerlaku: "2024-10-10" },
  { id: "D-3", nama: "Citra Lestari", noDosimeter: "DS-112", masaBerlaku: "2024-09-25" },
];

const daysTo = (dateStr: string) => {
  const today = new Date();
  const target = new Date(dateStr);
  const diff = Math.ceil((target.getTime() - new Date(today.toDateString()).getTime()) / (1000 * 60 * 60 * 24));
  return diff;
};

const RadiographyDosimeterSakuDashboard: React.FC = () => {
  ...
  const [form, setForm] = useState<{ nama: string; noDosimeter: string; masaBerlaku: string }>({ nama: '', noDosimeter: '', masaBerlaku: '' });
  const [showDelete, setShowDelete] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<Item | null>(null);

  const openAdd = () => {
    setEditing(null);
    setForm({ nama: '', noDosimeter: '', masaBerlaku: '' });
    setShowForm(true);
  };

  const openEdit = (row: Item) => {
    setEditing(row);
    setForm({ nama: row.nama, noDosimeter: row.noDosimeter, masaBerlaku: row.masaBerlaku });
    setShowForm(true);
  };

  const genId = () => {
    // Generate next id like D-<n>
  ...
  const saveForm = () => {
    const payload: Item = {
      ...
      nama: form.nama.trim(),
      noDosimeter: form.noDosimeter.trim(),
      masaBerlaku: form.masaBerlaku,
    };
    if (!payload.nama || !payload.noDosimeter || !payload.masaBerlaku) {
      alert('Nama, No. Dosimeter, dan Masa Berlaku wajib diisi');
      return;
    }
    ...
    <>
      <div className="min-h-screen bg-gray-50">
        <div className="bg-gradient-to-r from-blue-50 to-white border-b border-gray-100">
          <div className="max-w-7xl mx-auto px-6 py-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">DOSIMETER SAKU</h1>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">No. Dosimeter</th>
                  ...
                  {paged.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className={danger ? "bg-red-50 hover:bg-red-100 transition-colors" : "hover:bg-gray-50 transition-colors"}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{row.noDosimeter}</td>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
                <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
                  <h2 className="text-xl font-bold text-gray-900">{editing ? 'Edit Dosimeter Saku' : 'Tambah Dosimeter Saku'}</h2>
                ...
                <div className="p-6 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">No. Dosimeter</label>
                      <input
                        value={form.noDosimeter}
                        onChange={(e)=>setForm(prev=>({...prev, noDosimeter: e.target.value}))}
                        className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                        placeholder="Contoh: DS-001"
                      />
                    </div>
                    <div>
          ...
          <ConfirmDeleteModal
            isOpen={showDelete}
            title="Hapus Data Dosimeter?"
            message={deleteTarget ? `Hapus data ${deleteTarget.nama} (${deleteTarget.noDosimeter})?` : ''}
...
export default RadiographyDosimeterSakuDashboard;
