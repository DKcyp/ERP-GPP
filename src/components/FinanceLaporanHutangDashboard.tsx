...
const FinanceLaporanHutangDashboard: React.FC = () => {
  const today = new Date();
  const [rows, setRows] = useState<HutangRow[]>([
    { id: 1, tanggal: "2025-09-01", vendor: "PT Jaya", noInvoice: "INV-001", jatuhTempo: "2025-09-30", dpp: 10000000, ppn: 1100000, total: 11100000, status: "Belum Bayar" },
    { id: 2, tanggal: "2025-09-05", vendor: "CV Maju", noInvoice: "INV-002", jatuhTempo: "2025-09-25", dpp: 5000000, ppn: 550000, total: 5550000, status: "Sebagian" },
  ]);
  ...
  const [editing, setEditing] = useState<HutangRow | null>(null);

  const filtered = useMemo(() => rows.filter(r =>
    (filterVendor ? r.vendor.toLowerCase().includes(filterVendor.toLowerCase()) : true) &&
    (filterStatus ? r.status === (filterStatus as any) : true) &&
    ([r.noInvoice, r.vendor].join(" ").toLowerCase().includes(search.toLowerCase()))
  ), [rows, filterVendor, filterStatus, search]);

  const startAdd = () => { setEditing({ id: 0, tanggal: new Date().toISOString().split('T')[0], vendor: "", noInvoice: "", jatuhTempo: new Date().toISOString().split('T')[0], dpp: 0, ppn: 0, total: 0, status: "Belum Bayar" }); setFormOpen(true); };
  const startEdit = (row: HutangRow) => { setEditing(row); setFormOpen(true); };
  const remove = (id: number) => setRows(prev => prev.filter(r => r.id !== id));
  const onSave = (data: HutangRow) => {
      ...
      <div className="bg-gradient-to-r from-blue-50 to-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 tracking-wide mb-2">AP - LAPORAN HUTANG</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">Finance</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">AP / Laporan Hutang</span>
              </nav>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Cari Vendor/Invoice</label>
              <div className="relative">
                <input value={search} onChange={e => setSearch(e.target.value)} className="block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm pr-9" placeholder="Ketik kata kunci..." />
                <Search className="h-4 w-4 text-gray-400 absolute right-2 top-1/2 -translate-y-1/2" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Filter Vendor</label>
              <input value={filterVendor} onChange={e => setFilterVendor(e.target.value)} className="block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Status</label>
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm appearance-none">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.map(row => (
                  <tr key={row.id}>
                    ...
                    <td className="px-4 py-2 text-sm">{new Date(row.jatuhTempo).toLocaleDateString('id-ID')}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white w-full max-w-2xl rounded-lg shadow-lg">
            <div className="flex items-center justify-between p-4 border-b">
              <h3 className="text-sm font-semibold">{editing.id ? 'Edit' : 'Tambah'} Hutang</h3>
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="date" value={editing.jatuhTempo} onChange={e => setEditing({ ...editing, jatuhTempo: e.target.value })} className="w-full border rounded px-2 py-1.5 text-sm" />
                </div>
                <div>
...
export default FinanceLaporanHutangDashboard;
