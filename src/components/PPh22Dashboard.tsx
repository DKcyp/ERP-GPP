...
import { PPh22Data } from '../types';
import EntryPPh22Modal from './EntryPPh22Modal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

const dummyData: PPh22Data[] = [
  { id: 1, namaPihak: 'PT Citra Bangun', npwp: '33.222.111.0-999.000', tanggal: '2024-04-30', pph22: 900000 },
];

const PPh22Dashboard: React.FC = () => {
  const [data, setData] = useState<PPh22Data[]>(dummyData);
  const [searchTerm, setSearchTerm] = useState('');

  const [isAddEditModalOpen, setIsAddEditModalOpen] = useState(false);
  const [itemToEdit, setItemToEdit] = useState<PPh22Data | null>(null);
  ...
  const handleEditClick = (item: PPh22Data) => { setItemToEdit(item); setIsAddEditModalOpen(true); };
  const handleSaveItem = (newItem: PPh22Data) => {
    if (newItem.id) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">PPh 22</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">Tax</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">PPh 22</span>
              </nav>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">PPh 22</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">{formatCurrency(item.pph22)}</td>
                ...
                {filteredData.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data PPh 22 yang ditemukan.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <EntryPPh22Modal isOpen={isAddEditModalOpen} onClose={handleCloseAddEditModal} onSave={handleSaveItem} itemToEdit={itemToEdit} />
