...
const QHSEDaftarIndukDokumenEksternalDashboard: React.FC = () => {
  const [documentData, setDocumentData] =
    useState<DocumentRecord[]>(initialDocumentData);
  ...
  const [selectedRecordForAction, setSelectedRecordForAction] =
    useState<DocumentRecord | null>(null); // New state for record to be approved/rejected
  ...
  const departments = useMemo(() => {
    const unique = Array.from(
      new Set(documentData.map((item) => item.departemen))
    );
    return unique.sort();
  }, [documentData]);
  ...
  const filteredData = useMemo(() => {
    return documentData.filter((doc) => {
      ...
      const matchesDepartemen =
        filterDepartemen === "" || doc.departemen === filterDepartemen;
  ...
  const handleApprove = (id: string) => {
    // Logic to update the document status to 'Approved'
    setDocumentData((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, status: "Valid", keterangan: "Approved by ISO System" }
          : item
      )
    );
  ...
  const handleReject = (id: string, reason: string) => {
    // Logic to update the document status to 'Rejected' with reason
    setDocumentData((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, status: "Expired", keterangan: `Rejected: ${reason}` }
          : item
      )
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        sampaiMasaBerlaku: e.target.value,
                      }))
                    }
                    disabled={modalMode === "view"}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-100"
      ...
      <ConfirmationModal
        ...
        onConfirm={() =>
          selectedRecordForAction && handleApprove(selectedRecordForAction.id)
        }
        title="Konfirmasi Approve Dokumen"
        message={`Apakah Anda yakin ingin menyetujui dokumen '${selectedRecordForAction?.namaDokumen}'?`}
        confirmText="Approve"
        cancelText="Batal"
      ...
      <ConfirmationModal
        ...
        onConfirm={(reason) =>
          ...
          reason &&
          handleReject(selectedRecordForAction.id, reason)
        }
        title="Konfirmasi Reject Dokumen"
        message={`Apakah Anda yakin ingin menolak dokumen '${selectedRecordForAction?.namaDokumen}'? Masukkan alasan penolakan.`}
        confirmText="Tolak"
        cancelText="Batal"
        isReject={true}
