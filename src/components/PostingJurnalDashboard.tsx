          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <nav className="text-sm text-gray-600">
                ...
                <span className="text-blue-600 font-medium">Posting Jurnal</span>
              </nav>
            </div>
                ...
                <tr>
                  <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedJurnals(new Set(filteredData.filter(j => !j.isPosted).map(j => j.id)));
                        } else {
        ...
        <div className="flex justify-end">
          <button
            ...
            className={`inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white ${
              selectedJurnals.size > 0 ? 'bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500' : 'bg-gray-400 cursor-not-allowed'
            } transition-colors`}
          >
            <CheckSquare className="h-5 w-5 mr-2" /> Posting Jurnal ({selectedJurnals.size})
          </button>
        </div>
      </div>
    </div>
