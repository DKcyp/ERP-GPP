...
import { 
  Search, Filter, Download, TrendingUp, TrendingDown, Package, 
  AlertTriangle, CheckCircle, Clock, BarChart3, Eye, Calendar,
...
const KartuStockDashboard: React.FC = () => {
  ...
  const totalNilaiStock = stockData.reduce((sum, item) => sum + item.nilaiTotal, 0);
  const totalItems = stockData.length;
  const lowStockItems = stockData.filter(item => item.status === 'Low Stock' || item.status === 'Critical').length;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Total Nilai Stock</p>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">Daftar Stock Barang</h3>
          </div>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">{item.kodeBarang} • {item.serialNumber}</div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.stokAwal} {item.satuan}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{item.stokAkhir} {item.satuan}</div>
                      <div className="text-xs text-gray-500">Min: {item.stokMinimal}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      Rp {item.nilaiTotal.toLocaleString('id-ID')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center p-6 border-b">
                <h3 className="text-xl font-semibold text-gray-900">Detail Pergerakan Stock</h3>
                <button
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {stockMovements.map((movement) => (
                        <tr key={movement.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 text-sm text-gray-900">{movement.tanggal}</td>
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              movement.jenis === 'Masuk' ? 'bg-green-100 text-green-800' : 
                              movement.jenis === 'Keluar' ? 'bg-red-100 text-red-800' : 
                              'bg-blue-100 text-blue-800'
                            }`}>
                              {movement.jenis}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-900">{movement.referensi}</td>
                          <td className="px-4 py-3 text-sm text-gray-900">{movement.qty}</td>
                          <td className="px-4 py-3 text-sm font-medium text-gray-900">{movement.saldo}</td>
                          <td className="px-4 py-3 text-sm text-gray-500">{movement.keterangan}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
