...
const NoSuratPerjanjianDashboard: React.FC = () => {
  const [data, setData] = useState<NoSuratEntry[]>([
    { id: "1", no: 1, noSurat: "PKS-001/HRD/2025", keterangan: "Perjanjian awal tahun" },
    { id: "2", no: 2, noSurat: "PKS-002/HRD/2025", keterangan: "Kerjasama vendor" },
  ]);
  ...
  const [editing, setEditing] = useState<NoSuratEntry | null>(null);
  const [form, setForm] = useState<{ noSurat: string; keterangan: string; file: File | null }>({ noSurat: "", keterangan: "", file: null });
  const [saving, setSaving] = useState(false);

  const [deleteOpen, setDeleteOpen] = useState(false);
  const [toDelete, setToDelete] = useState<NoSuratEntry | null>(null);
  ...
  const openAdd = () => { setEditing(null); setForm({ noSurat: "", keterangan: "", file: null }); setIsFormOpen(true); };
  const openEdit = (row: NoSuratEntry) => { setEditing(row); setForm({ noSurat: row.noSurat, keterangan: row.keterangan || "", file: null }); setIsFormOpen(true); };
  ...
  const saveForm = async (e?: React.FormEvent) => {
    ...
    if (!form.noSurat.trim()) return;
    setSaving(true);
    await new Promise(r => setTimeout(r, 400));
    const fileName = form.file?.name;
    const fileUrl = form.file ? URL.createObjectURL(form.file) : undefined;
    if (editing) {
      setData(prev => prev.map(x => x.id === editing.id ? { ...x, noSurat: form.noSurat.trim(), keterangan: form.keterangan.trim(), fileName: fileName || x.fileName, fileUrl: fileUrl || x.fileUrl } : x));
    } else {
      const newId = (data.length + 1).toString();
      const newRow: NoSuratEntry = { id: newId, no: 1, noSurat: form.noSurat.trim(), keterangan: form.keterangan.trim(), fileName, fileUrl };
  ...
  const requestDelete = (row: NoSuratEntry) => { setToDelete(row); setDeleteOpen(true); };
  const confirmDelete = () => {
    if (!toDelete) return;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-8">No. Surat Perjanjian</h1>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div>
              <input value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} placeholder="Cari no.surat / keterangan" className="px-3 py-2 border border-gray-300 rounded-md" />
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {pageData.map((row, index) => (
                  <tr key={row.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? "bg-white" : "bg-gray-25"} ${animateRows ? "animate-in fade-in slide-in-from-bottom-2" : "opacity-0"}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : "0ms", animationFillMode: "forwards" }}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{row.noSurat}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.keterangan || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {row.fileUrl ? (
                        <a href={row.fileUrl} download={row.fileName || true} className="inline-flex items-center gap-1 rounded bg-emerald-600 px-3 py-1 text-xs font-medium text-white hover:bg-emerald-700">Download</a>
                      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setIsFormOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
              <h2 className="text-xl font-bold text-gray-900">{editing ? 'Edit' : 'Tambah'} No. Surat</h2>
            ...
            <form onSubmit={saveForm} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">No. Surat <span className="text-red-500">*</span></label>
                <input value={form.noSurat} onChange={(e) => setForm(prev => ({ ...prev, noSurat: e.target.value }))} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Contoh: PKS-001/HRD/2025" required />
              </div>
              <div>
      ...
      {/* Delete Modal */}
      <ConfirmDeleteModal isOpen={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={confirmDelete} itemName={toDelete?.noSurat} />
    </div>
  );
};

export default NoSuratPerjanjianDashboard;
