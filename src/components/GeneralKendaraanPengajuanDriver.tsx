...
const seed: PengajuanDriverItem[] = [
  { id: crypto.randomUUID(), tanggal: "2025-09-01", namaDriver: "Budi", tujuan: "Customer A", kendaraan: "Mobil A", status: "Pengajuan", keterangan: "Urgent" },
];

const GeneralKendaraanPengajuanDriver: React.FC = () => {
  const [rows, setRows] = useState<PengajuanDriverItem[]>(seed);
  const [q, setQ] = useState("");
  const [statusFilter, setStatusFilter] = useState<"" | Status>("");
  const [isOpen, setIsOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<PengajuanDriverItem>({ id: "", tanggal: "", namaDriver: "", tujuan: "", kendaraan: "", status: "Pengajuan", keterangan: "" });
  ...
  const openAdd = () => {
    setEditingId(null);
    setForm({ id: crypto.randomUUID(), tanggal: "", namaDriver: "", tujuan: "", kendaraan: "", status: "Pengajuan", keterangan: "" });
    setIsOpen(true);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">General - Kendaraan - Pengajuan Driver</h1>
            <div className="text-xs text-gray-600">General › Kendaraan › Pengajuan Driver</div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={(e)=> e.target===e.currentTarget && setIsOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
              <h2 className="text-lg font-semibold">{editingId? 'Edit':'Tambah'} Pengajuan</h2>
            ...
            <form onSubmit={saveForm} className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea value={form.keterangan} onChange={e=>setForm(f=>({...f, keterangan:e.target.value}))} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"/>
                </div>
              </div>
...
export default GeneralKendaraanPengajuanDriver;
