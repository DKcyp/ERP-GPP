...
import { SKPKBData } from '../types';
import EntrySKPKBModal from './EntrySKPKBModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

const dummyData: SKPKBData[] = [
  { id: 1, namaPihak: 'PT Nusantara Abadi', npwp: '12.345.678.9-000.111', tanggal: '2024-08-31', skpkb: 25000000 },
];

const SKPKBDashboard: React.FC = () => {
  const [data, setData] = useState<SKPKBData[]>(dummyData);
  const [searchTerm, setSearchTerm] = useState('');

  const [isAddEditModalOpen, setIsAddEditModalOpen] = useState(false);
  const [itemToEdit, setItemToEdit] = useState<SKPKBData | null>(null);
  ...
  const handleEditClick = (item: SKPKBData) => { setItemToEdit(item); setIsAddEditModalOpen(true); };
  const handleSaveItem = (newItem: SKPKBData) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">SKPKB</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">Tax</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">SKPKB</span>
              </nav>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Nilai SKPKB</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">{formatCurrency(item.skpkb)}</td>
                ...
                {filteredData.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data SKPKB yang ditemukan.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <EntrySKPKBModal isOpen={isAddEditModalOpen} onClose={handleCloseAddEditModal} onSave={handleSaveItem} itemToEdit={itemToEdit} />
