...
interface PotonganLainEntry {
  id: string;
  no: number;
  kodePegawai: string;
  namaPegawai: string;
  jenisPegawai: string;
  periode: string; // YYYY-MM or YYYY-MM-DD
...
const PotonganLainDashboard: React.FC = () => {
  ...
  const [editing, setEditing] = useState<PotonganLainEntry | null>(null);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<PotonganLainEntry | null>(null);
  const [kodePegawai, setKodePegawai] = useState("");
  const [namaPegawai, setNamaPegawai] = useState("");

  const kodePegawaiOptions = ["EMP001", "EMP002", "EMP003", "EMP004", "EMP005"];

  const [data, setData] = useState<PotonganLainEntry[]>([
    { id: "1", no: 1, kodePegawai: "EMP001", namaPegawai: "Ahmad Fauzi", jenisPegawai: "Tetap", periode: "2025-09", totalPotongan: "Rp 350.000", keterangan: "Denda keterlambatan" },
    { id: "2", no: 2, kodePegawai: "EMP002", namaPegawai: "Siti Nurhaliza", jenisPegawai: "Kontrak", periode: "2025-09", totalPotongan: "Rp 120.000", keterangan: "Lain-lain" },
  ]);
  ...
  const handleOpenEdit = (row: PotonganLainEntry) => { setEditing(row); setIsModalOpen(true); };
  ...
  const filtered = data.filter(item => {
    ...
    const matchesSearch =
      ...
      item.totalPotongan.toLowerCase().includes(q);
  ...
  const sumPotongan = (f: PotonganGajiFormData) => {
    const nums = [
      f.bpjsTk, f.bpjsKes, f.pph21, f.ca, f.potonganMess,
      ...f.potonganItems.map(i => i.nominal)
    ]
      .filter(Boolean)
  ...
  const handleSave = (f: PotonganGajiFormData) => {
    if (editing) {
      setData(prev => prev.map(r => r.id === editing.id ? {
        id: editing.id,
        no: r.no,
        kodePegawai: f.kodePegawai,
        namaPegawai: f.namaPegawai,
        jenisPegawai: f.jenisPegawai,
        periode: f.periode,
        totalPotongan: sumPotongan(f),
        keterangan: f.keterangan,
      } : r));
    } else {
      const newId = (data.length + 1).toString();
      const newRow: PotonganLainEntry = {
        id: newId,
        no: 1,
        kodePegawai: f.kodePegawai,
        namaPegawai: f.namaPegawai,
  ...
  const requestDelete = (row: PotonganLainEntry) => { setItemToDelete(row); setDeleteModalOpen(true); };
  const confirmDelete = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-8">Potongan Lain-lain</h1>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Potongan</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {pageData.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.totalPotongan}</td>
      ...
      {/* Modals */}
      <PotonganGajiModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditing(null); }} onSave={handleSave} />
...
export default PotonganLainDashboard;
