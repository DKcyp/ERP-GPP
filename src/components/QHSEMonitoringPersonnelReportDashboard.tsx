...
interface PersonnelMonitoring {
  ...
  namaPersonel: string;
  lokasiKerja: string;
  jenisReport: string; // Changed from aktivitasQHSE
  status: "Aktif" | "Training" | "Cuti" | "Selesai Project";
  namaClient: string; // Changed from keterangan
...
const samplePersonnelData: PersonnelMonitoring[] = [
  // Januari 2025
  {
    ...
    lokasiKerja: "Cilegon",
    jenisReport: "Ultrasonic Testing",
    status: "Aktif",
    namaClient: "PT. Bakrie Metal Industri",
    periode: "2025-01",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    jenisReport: "Ultrasonic Testing",
    status: "Aktif",
    namaClient: "PT. Freyssinet Total Technology",
    periode: "2025-01",
  },
  {
    id: 8,
  ...
  },
  {
    ...
    jenisReport: "Ultrasonic Testing",
    status: "Aktif",
    namaClient: "CV. Tawakkal Indo Buana Mas",
    periode: "2025-02",
  },
  {
    id: 11,
...
const QHSEMonitoringPersonnelReportDashboard: React.FC = () => {
  const [personnelData, setPersonnelData] =
    useState<PersonnelMonitoring[]>(samplePersonnelData);
  const [searchTerm, setSearchTerm] = useState("");
  const [filterLokasi, setFilterLokasi] = useState("");
  const [filterPeriode, setFilterPeriode] = useState("");
  const [sortField, setSortField] =
    useState<keyof PersonnelMonitoring>("tanggal");
  ...
  const [selectedPersonnel, setSelectedPersonnel] =
    useState<PersonnelMonitoring | null>(null);
  ...
  const uniqueLokasi = useMemo(
    () => [...new Set(personnelData.map((item) => item.lokasiKerja))],
    [personnelData]
  );
  const uniquePeriode = useMemo(
    () =>
      [...new Set(personnelData.map((item) => item.periode))].sort().reverse(),
    [personnelData]
  );
  ...
  const filteredData = useMemo(() => {
    let filtered = personnelData.filter((item) => {
      ...
      const matchesLokasi =
        filterLokasi === "" || item.lokasiKerja === filterLokasi;
      const matchesPeriode =
        filterPeriode === "" || item.periode === filterPeriode;

      return matchesSearch && matchesLokasi && matchesPeriode;
    });
  ...
  const handleInputChange = (
    field: keyof PersonnelMonitoring,
    value: string
  ) => {
    setFormData((prev) => {
      ...
      if (field === "tanggal" && value) {
        ...
        updated.periode = `${date.getFullYear()}-${(date.getMonth() + 1)
          .toString()
          .padStart(2, "0")}`;
      }
      ...
      setPersonnelData((prev) =>
        prev.map((p) =>
          p.id === selectedPersonnel.id
            ? { ...(formData as PersonnelMonitoring), id: selectedPersonnel.id }
            : p
        )
      );
    }
  ...
  const confirmDelete = () => {
    if (selectedPersonnel) {
      setPersonnelData((prev) =>
        prev.filter((p) => p.id !== selectedPersonnel.id)
      );
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="bg-white-500 text-dark">
                <tr>
                  ...
                  <th className="px-4 py-3 border border-gray-400 text-center font-semibold">
                    Aksi
                  </th>
                </tr>
              </thead>
              <tbody>
                {paginatedData.map((item, index, arr) => {
                  const showMonth =
                    index === 0 || item.periode !== arr[index - 1]?.periode;
                    ...
                    <React.Fragment key={item.id}>
                      {showMonth && (
                        <tr>
                          <td
                            colSpan={10}
                            className="bg-orange-300 text-center font-bold border border-gray-400 py-2 text-orange-900"
                          >
                            {formatDate(item.periode + "-01")
                              .split(" ")
                              .slice(1)
                              .join(" ")}
                          </td>
                      ...
                      <tr
                        className={`${rowClass} ${getStatusColor(
                          item.status
                      ...
                      >
                        ...
                        <td className="px-4 py-3 border border-gray-300 font-mono text-xs">
                          {item.nomorReport}
                        </td>
                        <td className="px-4 py-3 border border-gray-300">
                          {item.nomorSO}
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("nomorSO", e.target.value)
                      }
                      disabled={modalMode === "view"}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent disabled:bg-gray-100"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("namaPersonel", e.target.value)
                    }
                    disabled={modalMode === "view"}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent disabled:bg-gray-100"
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
              <div className="p-6">
                ...
                <p className="text-gray-600 mb-6">
                  ...
                  <strong>{selectedPersonnel.namaPersonel}</strong>? This action
                  cannot be undone.
                </p>
