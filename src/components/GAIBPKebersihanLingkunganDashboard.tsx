...
import { Search, Plus, FileSpreadsheet, FileText, Clock, Edit, Trash2, Calendar, MapPin, User } from 'lucide-react';
import KebersihanLingkunganModal, { KebersihanLingkunganForm } from './KebersihanLingkunganModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface CleanItem extends KebersihanLingkunganForm { id: string }

const seedData = (): CleanItem[] => [
  { id: '1', tanggal: '2025-09-01', area: 'Lobby', aktivitas: 'Pengepelan & disinfeksi', petugas: 'Slamet', status: 'Done', skor: '95', catatan: 'Bersih' },
  { id: '2', tanggal: '2025-09-03', area: 'Toilet Lantai 2', aktivitas: 'Pembersihan kaca & sanitasi', petugas: 'Wati', status: 'In Progress', skor: '', catatan: '' },
  { id: '3', tanggal: '2025-09-05', area: 'Ruang Rapat', aktivitas: 'Vacuum karpet', petugas: 'Budi', status: 'Scheduled', skor: '', catatan: 'Jadwal rutin' },
];

const statusPill = (status: CleanItem['status']) => {
  switch (status) {
...
const GAIBPKebersihanLingkunganDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<CleanItem[]>(seedData());
  const [animateRows, setAnimateRows] = useState(false);

  // Modals
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<CleanItem | null>(null);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<CleanItem | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    const p = petugas.trim().toLowerCase();
    return data.filter(d => {
      const matchText = q ? `${d.aktivitas} ${d.catatan ?? ''}`.toLowerCase().includes(q) : true;
      const matchArea = a ? d.area.toLowerCase().includes(a) : true;
      const matchPetugas = p ? d.petugas.toLowerCase().includes(p) : true;
      const matchStatus = status ? d.status === (status as CleanItem['status']) : true;
      const dt = new Date(d.tanggal);
      const from = dateFrom ? new Date(dateFrom) : null;
      const to = dateTo ? new Date(dateTo) : null;
      const matchDate = (!from || dt >= from) && (!to || dt <= to);
      return matchText && matchArea && matchPetugas && matchStatus && matchDate;
    });
  }, [data, search, area, petugas, status, dateFrom, dateTo]);
  ...
  const handleSave = (form: KebersihanLingkunganForm) => {
    if (editingItem) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">KEBERSIHAN LINGKUNGAN KERJA</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Kebersihan Lingkungan Kerja</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6 mb-6">
            ...
            <div className="space-y-2 lg:col-span-2">
              <label className="block text-xs font-medium text-gray-700">Cari (Aktivitas/Catatan)</label>
              <div className="relative">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Contoh: Disinfeksi / Kaca / Karpet" />
            ...
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Area/Ruangan</label>
              <div className="relative">
                <input type="text" value={area} onChange={(e) => setArea(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Lobby / Lantai 2 / Toilet" />
            ...
            {/* Petugas */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Petugas/Vendor</label>
              <div className="relative">
                <input type="text" value={petugas} onChange={(e) => setPetugas(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Nama petugas/vendor" />
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Area/Ruangan</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Aktivitas</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Petugas/Vendor</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Status</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Skor</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'} ${animateRows ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : '0ms', animationFillMode: 'forwards' }}>
                    ...
                    <td className="px-2 py-1 font-medium text-gray-900">{item.area}</td>
                    <td className="px-2 py-1 text-gray-700 truncate max-w-[260px]" title={item.aktivitas}>{item.aktivitas}</td>
                    <td className="px-2 py-1 text-gray-700">{item.petugas}</td>
                    ...
                    <td className="px-2 py-1 text-gray-700">{item.skor || '-'}</td>
                    <td className="px-2 py-1 text-gray-700">{item.catatan || '-'}</td>
      ...
      <KebersihanLingkunganModal
        ...
        initialData={editingItem ? {
          ...
          aktivitas: editingItem.aktivitas,
          petugas: editingItem.petugas,
          status: editingItem.status,
          skor: editingItem.skor,
          catatan: editingItem.catatan,
        } : null}
        title={editingItem ? 'Edit Kegiatan Kebersihan' : 'Tambah Kegiatan Kebersihan'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.area ?? ''} - ${deleteTarget?.aktivitas ?? ''}`}
      />
    </div>
  );
};

export default GAIBPKebersihanLingkunganDashboard;
