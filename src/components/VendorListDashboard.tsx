...
const VendorListDashboard: React.FC = () => {
  const [searchNamaVendor, setSearchNamaVendor] = useState('');
  const [searchNamaPIC, setSearchNamaPIC] = useState('');
  ...
  const [itemToDelete, setItemToDelete] = useState<VendorData | null>(null);
  const [sortField, setSortField] = useState<keyof VendorData | null>(null);
  ...
  const [vendorData, setVendorData] = useState<VendorData[]>([
    {
      id: '1',
      no: 1,
      tglPendaftaranVendor: '01-02-2024',
    ...
    },
    {
      id: '3',
      no: 3,
      tglPendaftaranVendor: '10-02-2025',
      namaVendor: 'UD Sukses Makmur',
      namaPIC: 'Cahyo Widodo',
      noTelp: '082112345678'
    },
    {
      id: '4',
      no: 4,
      tglPendaftaranVendor: '15-02-2024',
  ...
  const handleAddVendor = (formData: VendorFormData) => {
    const newVendor: VendorData = {
      id: (vendorData.length + 1).toString(),
      no: vendorData.length + 1,
      tglPendaftaranVendor: new Date().toLocaleDateString('id-ID', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric'
      }),
    ...
    setVendorData(prev => [newVendor, ...prev.map(v => ({ ...v, no: v.no + 1 }))]);
  };
  ...
  const handleConfirmDelete = () => {
    if (itemToDelete) {
      setVendorData(prev => prev.filter(v => v.id !== itemToDelete.id));
  ...
  const filteredData = vendorData.filter(item => {
    ...
    const matchesNamaPIC = item.namaPIC.toLowerCase().includes(searchNamaPIC.toLowerCase());
    
    return matchesNamaVendor && matchesNamaPIC;
  });
              ...
              <div className="space-y-2">
                ...
                <div className="flex space-x-2">
                  <input
                    type="text"
                    value={searchNamaPIC}
                    onChange={(e) => setSearchNamaPIC(e.target.value)}
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-cyan-500 focus:border-transparent text-sm"
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th 
                    className="px-4 py-3 text-left text-sm font-medium text-gray-700 cursor-pointer hover:bg-gray-100 transition-colors"
                    onClick={() => handleSort('tglPendaftaranVendor')}
                  >
                    <div className="flex items-center space-x-1">
                      <span>Tgl Pendaftaran Vendor</span>
                      {sortField === 'tglPendaftaranVendor' && (
                        <ArrowUp className={`h-3 w-3 transition-transform ${sortDirection === 'desc' ? 'rotate-180' : ''}`} />
                      )}
                    </div>
                  </th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{item.tglPendaftaranVendor}</td>
                    <td className="px-4 py-3 text-sm text-gray-900 font-medium">{item.namaVendor}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{item.namaPIC}</td>
...
export default VendorListDashboard;
