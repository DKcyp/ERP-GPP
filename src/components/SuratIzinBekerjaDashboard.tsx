            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">No. SIB</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Masa Berlaku</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? "bg-white" : "bg-gray-25"} ${animateRows ? "animate-in fade-in slide-in-from-bottom-2" : "opacity-0"}`} style={{ animationDelay: animateRows ? `${index * 80}ms` : "0ms", animationFillMode: "forwards" }}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{item.tanggalTerbit} - {item.berlakuSampai}</td>
