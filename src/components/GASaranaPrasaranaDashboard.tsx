...
import { Search, Plus, FileSpreadsheet, FileText, Clock, Edit, Trash2, MapPin } from 'lucide-react';
import GASaranaPrasaranaModal, { SaprasForm } from './GASaranaPrasaranaModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface SaprasItem extends SaprasForm { id: string }

const seedData = (): SaprasItem[] => [
  { id: '1', namaAset: 'AC Lantai 2', kategori: 'Peralatan', lokasi: 'Kantor Pusat', kondisi: 'Baik', jadwalService: '2025-10-01', statusService: 'Scheduled', keterangan: 'Cuci berkala' },
  { id: '2', namaAset: 'Genset', kategori: 'Peralatan', lokasi: 'Gudang A', kondisi: 'Rusak Ringan', jadwalService: '2025-09-20', statusService: 'Overdue', keterangan: 'Sparepart menunggu' },
  { id: '3', namaAset: 'Ruang Meeting', kategori: 'Gedung', lokasi: 'Lantai 3', kondisi: 'Baik', jadwalService: '2025-09-30', statusService: 'Scheduled' },
];

const GASaranaPrasaranaDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<SaprasItem[]>(seedData());
  const [animateRows, setAnimateRows] = useState(false);

  // Modals
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<SaprasItem | null>(null);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SaprasItem | null>(null);
  ...
  const filtered = useMemo(() => {
    const q = searchAset.trim().toLowerCase();
    return data.filter(d => {
      const matchAset = q ? d.namaAset.toLowerCase().includes(q) || d.lokasi.toLowerCase().includes(q) : true;
      const matchKat = kategori ? d.kategori === (kategori as SaprasItem['kategori']) : true;
      const matchStat = statusService ? d.statusService === (statusService as SaprasItem['statusService']) : true;
      return matchAset && matchKat && matchStat;
    });
  }, [data, searchAset, kategori, statusService]);
  ...
  const handleSave = (form: SaprasForm) => {
    if (editingItem) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
            ...
            <div className="space-y-2 lg:col-span-2">
              <label className="block text-xs font-medium text-gray-700">Cari Aset / Lokasi</label>
              <div className="relative">
                <input
                  type="text"
                  value={searchAset}
                  onChange={(e) => setSearchAset(e.target.value)}
                  className="w-full pl-3 pr-8 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs"
                  placeholder="Contoh: AC / Kantor Pusat"
                />
            ...
            {/* Status Service */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Status Service</label>
              <select
                value={statusService}
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Nama Aset</th>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Kondisi</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Jadwal Service</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Status Service</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 font-medium text-gray-900">{item.namaAset}</td>
                    <td className="px-2 py-1 text-gray-700">{item.kategori}</td>
                    <td className="px-2 py-1 text-gray-700 flex items-center gap-1"><MapPin className="h-3 w-3 text-gray-400" />{item.lokasi}</td>
                    <td className="px-2 py-1 text-gray-700">{item.kondisi}</td>
                    <td className="px-2 py-1 text-gray-700">{new Date(item.jadwalService).toLocaleDateString('id-ID')}</td>
                    <td className="px-2 py-1">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${item.statusService === 'Done' ? 'bg-green-100 text-green-800 border-green-200' : item.statusService === 'Overdue' ? 'bg-red-100 text-red-800 border-red-200' : 'bg-yellow-100 text-yellow-800 border-yellow-200'}`}>
                        {item.statusService}
                      </span>
                    </td>
      ...
      <GASaranaPrasaranaModal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setEditingItem(null); }}
        onSave={handleSave}
        initialData={editingItem ? {
          namaAset: editingItem.namaAset,
          kategori: editingItem.kategori,
          lokasi: editingItem.lokasi,
          kondisi: editingItem.kondisi,
          jadwalService: editingItem.jadwalService,
          statusService: editingItem.statusService,
          keterangan: editingItem.keterangan,
        } : null}
        title={editingItem ? 'Edit Sarana/Prasarana' : 'Tambah Sarana/Prasarana'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.namaAset ?? ''} - ${deleteTarget?.lokasi ?? ''}`}
      />
    </div>
  );
};

export default GASaranaPrasaranaDashboard;
