...
export interface BBKDetailItem {
  id: number;
  kodeBank: string;
...
export interface BBKKeluarFormData {
  id?: number;
  noBBK: string;
  tglBBK: Date | null;
  namaDivisi: string;
  ...
  bayarKepada: string;
  keterangan: string;
  detailItems: BBKDetailItem[];
  total: number;
}

interface FinanceBuktiBankKeluarModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: BBKKeluarFormData) => void;
  initialData?: BBKKeluarFormData | null;
  title: string;
}

const FinanceBuktiBankKeluarModal: React.FC<FinanceBuktiBankKeluarModalProps> = ({ isOpen, onClose, onSave, initialData, title }) => {
  const today = new Date();

  const initialEmpty: BBKKeluarFormData = {
    noBBK: `BBK-${new Date().getFullYear()}-${(new Date().getMonth() + 1).toString().padStart(2, '0')}-001`,
    tglBBK: today,
    namaDivisi: '',
    jenisTransaksi: '',
    noDokumen: '',
  ...
  const [formData, setFormData] = useState<BBKKeluarFormData>(initialEmpty);
  ...
  // Bank meta + options with (norek - alamat)
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      setFormData(initialData ? { ...initialData, tglBBK: initialData.tglBBK ? new Date(initialData.tglBBK) : today, total: calcTotal(initialData.detailItems) } : initialEmpty);
      setErrors({});
    }
    return () => {
  ...
  const calcTotal = (items: BBKDetailItem[]) => items.reduce((sum, it) => sum + (Number(it.nominal) || 0), 0);
  ...
  const handleDetailChange = (id: number, field: keyof BBKDetailItem, value: any) => {
  ...
  const validate = () => {
    ...
    if (!formData.noBBK.trim()) e.noBBK = 'No BBK wajib diisi';
    if (!formData.tglBBK) e.tglBBK = 'Tanggal BBK wajib diisi';
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-160px)]">
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">No BBK <span className="text-red-500">*</span></label>
                <input type="text" value={formData.noBBK} onChange={e => setFormData(prev => ({ ...prev, noBBK: e.target.value }))} className={`block w-full border rounded-lg px-4 py-2 text-sm ${errors.noBBK ? 'border-red-300' : 'border-gray-300'}`} />
                {errors.noBBK && <p className="mt-1 text-sm text-red-600">{errors.noBBK}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Tgl BBK <span className="text-red-500">*</span></label>
                <DatePicker selected={formData.tglBBK} onChange={(date: Date | null) => setFormData(prev => ({ ...prev, tglBBK: date }))} dateFormat="dd/MM/yyyy" className={`block w-full border rounded-lg px-4 py-2 text-sm ${errors.tglBBK ? 'border-red-300' : 'border-gray-300'}`} />
                {errors.tglBBK && <p className="mt-1 text-sm text-red-600">{errors.tglBBK}</p>}
              </div>
              <div>
...
export default FinanceBuktiBankKeluarModal;
