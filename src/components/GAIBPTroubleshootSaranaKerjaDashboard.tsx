...
import { Search, Plus, FileSpreadsheet, FileText, Clock, Edit, Trash2, Calendar, MapPin, Tool } from 'lucide-react';
import TroubleshootSaranaKerjaModal, { TroubleshootSaranaKerjaForm } from './TroubleshootSaranaKerjaModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface TroubleItem extends TroubleshootSaranaKerjaForm { id: string }

const seedData = (): TroubleItem[] => [
  { id: '1', tanggal: '2025-09-01', pelapor: 'Rudi', lokasi: 'Ruang Server', aset: 'AC-Server-01', kategori: 'AC', deskripsi: 'AC tidak dingin', prioritas: 'High', status: 'Open', penangan: 'Andi', solusi: '' },
  { id: '2', tanggal: '2025-09-03', pelapor: 'Sari', lokasi: 'Lantai 2', aset: 'Panel Listrik 2F', kategori: 'Listrik', deskripsi: 'MCB sering turun', prioritas: 'Critical', status: 'In Progress', penangan: 'Budi', solusi: '' },
  { id: '3', tanggal: '2025-09-05', pelapor: 'Budi', lokasi: 'Lobby', aset: 'Automatic Door', kategori: 'Fasilitas', deskripsi: 'Pintu otomatis macet', prioritas: 'Medium', status: 'Resolved', penangan: 'Cici', solusi: 'Kalibrasi sensor' },
];

const statusPill = (status: TroubleItem['status']) => {
  switch (status) {
...
const priorityPill = (p: TroubleItem['prioritas']) => {
  switch (p) {
...
const GAIBPTroubleshootSaranaKerjaDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<TroubleItem[]>(seedData());
  ...
  const [deleteTarget, setDeleteTarget] = useState<TroubleItem | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d => {
      const matchText = q ? `${d.pelapor} ${d.lokasi} ${d.aset} ${d.deskripsi}`.toLowerCase().includes(q) : true;
      const matchKat = kategori ? d.kategori === (kategori as TroubleItem['kategori']) : true;
      const matchPri = prioritas ? d.prioritas === (prioritas as TroubleItem['prioritas']) : true;
      const matchStat = status ? d.status === (status as TroubleItem['status']) : true;
      const dt = new Date(d.tanggal);
      const from = dateFrom ? new Date(dateFrom) : null;
      const to = dateTo ? new Date(dateTo) : null;
      const matchDate = (!from || dt >= from) && (!to || dt <= to);
      return matchText && matchKat && matchPri && matchStat && matchDate;
    });
  }, [data, search, kategori, prioritas, status, dateFrom, dateTo]);
  ...
  const handleSave = (form: TroubleshootSaranaKerjaForm) => {
    if (editingItem) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">PENANGAN TROUBLESHOOT SISTEM SARANA KERJA</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Troubleshoot Sarana Kerja</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6 mb-6">
            ...
            <div className="space-y-2 lg:col-span-2">
              <label className="block text-xs font-medium text-gray-700">Cari (Pelapor/Lokasi/Aset/Keluhan)</label>
              <div className="relative">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Contoh: AC / Panel / Nama Pelapor" />
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Pelapor</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Lokasi</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Aset</th>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Penangan</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Solusi</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'} ${animateRows ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : '0ms', animationFillMode: 'forwards' }}>
                    ...
                    <td className="px-2 py-1 font-medium text-gray-900">{item.pelapor}</td>
                    <td className="px-2 py-1 text-gray-700">{item.lokasi}</td>
                    <td className="px-2 py-1 text-gray-700">{item.aset}</td>
                    ...
                    <td className="px-2 py-1 text-gray-700 truncate max-w-[240px]" title={item.deskripsi}>{item.deskripsi}</td>
                    <td className="px-2 py-1 text-gray-700">{item.penangan}</td>
                    <td className="px-2 py-1 text-gray-700">{item.solusi || '-'}</td>
      ...
      <TroubleshootSaranaKerjaModal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setEditingItem(null); }}
        onSave={handleSave}
        initialData={editingItem ? {
          tanggal: editingItem.tanggal,
          pelapor: editingItem.pelapor,
          lokasi: editingItem.lokasi,
          aset: editingItem.aset,
          kategori: editingItem.kategori,
          deskripsi: editingItem.deskripsi,
          prioritas: editingItem.prioritas,
          status: editingItem.status,
          penangan: editingItem.penangan,
          solusi: editingItem.solusi,
        } : null}
        title={editingItem ? 'Edit Ticket Troubleshoot Sarana Kerja' : 'Tambah Ticket Troubleshoot Sarana Kerja'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.aset ?? ''} - ${deleteTarget?.lokasi ?? ''}`}
      />
    </div>
  );
};

export default GAIBPTroubleshootSaranaKerjaDashboard;
