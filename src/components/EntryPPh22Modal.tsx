...
interface EntryPPh22ModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: PPh22Data) => void;
  itemToEdit?: PPh22Data | null;
}

const EntryPPh22Modal: React.FC<EntryPPh22ModalProps> = ({ isOpen, onClose, onSave, itemToEdit }) => {
  const initialFormState: PPh22Data = {
    id: 0,
    namaPihak: '',
    npwp: '',
    tanggal: '',
  ...
  const [formData, setFormData] = useState<PPh22Data>(initialFormState);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({ ...prev, [name]: name === 'pph22' ? parseFloat(value) || 0 : value }));
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 fade-in-0 duration-300" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">{itemToEdit ? 'Edit Data PPh 22' : 'Tambah Data PPh 22'}</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label htmlFor="pph22" className="block text-sm font-medium text-gray-700 mb-1">Jumlah PPh 22</label>
            <div className="relative">
              <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
              <input type="number" id="pph22" name="pph22" value={formData.pph22} onChange={handleChange} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" placeholder="Masukkan jumlah PPh 22" min={0} required />
            </div>
          </div>
...
export default EntryPPh22Modal;
