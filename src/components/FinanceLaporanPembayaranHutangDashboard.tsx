...
import { Clock, FileSpreadsheet, FileDown, Search, Eye } from "lucide-react";
import PembayaranHutangDetailModal from "./PembayaranHutangDetailModal";
...
const FinanceLaporanPembayaranHutangDashboard: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      const okVendor = searchVendor
        ? r.vendor.toLowerCase().includes(searchVendor.toLowerCase())
        : true;
      const okInv = searchNoPo
        ? r.noPo.toLowerCase().includes(searchNoPo.toLowerCase())
        : true;
      const okStatus = status ? r.status === (status as any) : true;
      const baseDate = r.tglPo || r.tanggalBayar;
      const okFrom = periodeDari
        ? new Date(baseDate) >= new Date(periodeDari.setHours(0, 0, 0, 0))
        : true;
      const okTo = periodeSampai
        ? new Date(baseDate) <=
          new Date(periodeSampai.setHours(23, 59, 59, 999))
        : true;
  ...
  const totalBayar = useMemo(
    () => filtered.reduce((s, r) => s + r.totalPembayaran, 0),
    [filtered]
  );
  ...
  const handleRowClick = (row: PembayaranRow) => {
    setSelectedRow(row);
    setIsModalOpen(true);
  };
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {new Date(row.tglPo).toLocaleDateString("id-ID")}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {new Date(row.tglJatuhTempo).toLocaleDateString("id-ID")}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {row.keterangan}
                    </td>
