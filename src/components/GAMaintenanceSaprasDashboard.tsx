...
import MaintenanceSaprasModal, { MaintenanceSaprasForm } from './MaintenanceSaprasModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface MaintenanceItem extends MaintenanceSaprasForm { id: string }

const seedData = (): MaintenanceItem[] => [
  { id: '1', namaAset: 'AC Lantai 2', lokasi: 'Kantor Pusat', jenisPekerjaan: 'Perawatan Berkala', tanggalMulai: '2025-09-01', tanggalSelesai: '2025-09-02', vendor: 'CV Sejuk Selalu', status: 'Done', biaya: '750.000', keterangan: 'Cuci + isi freon' },
  { id: '2', namaAset: 'Genset', lokasi: 'Gudang A', jenisPekerjaan: 'Perbaikan', tanggalMulai: '2025-09-10', tanggalSelesai: '2025-09-15', vendor: 'PT Tenaga Prima', status: 'In Progress', biaya: '4.500.000', keterangan: 'Ganti aki + servis' },
  { id: '3', namaAset: 'Lift', lokasi: 'Lantai 1-4', jenisPekerjaan: 'Perawatan Berkala', tanggalMulai: '2025-09-20', tanggalSelesai: '2025-09-20', vendor: 'PT Angkat Jaya', status: 'Scheduled', biaya: '2.000.000', keterangan: 'Kontrak rutin' },
];

const statusPill = (status: MaintenanceItem['status']) => {
  switch (status) {
...
const GAMaintenanceSaprasDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<MaintenanceItem[]>(seedData());
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d => {
      const matchText = q ? `${d.namaAset} ${d.lokasi} ${d.jenisPekerjaan}`.toLowerCase().includes(q) : true;
      const matchStatus = statusFilter ? d.status === (statusFilter as MaintenanceItem['status']) : true;
      const start = dateFrom ? new Date(dateFrom) : null;
      const end = dateTo ? new Date(dateTo) : null;
      const dStart = new Date(d.tanggalMulai);
      const dEnd = new Date(d.tanggalSelesai || d.tanggalMulai);
      const matchDate = (!start || dEnd >= start) && (!end || dStart <= end);
      return matchText && matchStatus && matchDate;
    });
  }, [data, searchAset, statusFilter, dateFrom, dateTo]);
  ...
  const handleSave = (form: MaintenanceSaprasForm) => {
    if (editingItem) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">MONITORING MAINTENANCE SARANA & PRASARANA</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Monitoring Maintenance Sapras</span>
              </nav>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
            {/* Cari Aset/Lokasi/Pekerjaan */}
            <div className="space-y-2 lg:col-span-2">
              <label className="block text-xs font-medium text-gray-700">Cari Aset / Lokasi / Pekerjaan</label>
              <div className="relative">
                <input
                  ...
                  placeholder="Contoh: AC / Perawatan / Kantor Pusat"
                />
            ...
            {/* Periode Sampai */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Tanggal Sampai</label>
              <div className="relative">
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Tanggal Mulai</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Tanggal Selesai</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'} ${animateRows ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : '0ms', animationFillMode: 'forwards' }}>
                    ...
                    <td className="px-2 py-1 text-gray-700">{item.tanggalSelesai ? new Date(item.tanggalSelesai).toLocaleDateString('id-ID') : '-'}</td>
      ...
      <MaintenanceSaprasModal
        ...
        initialData={editingItem ? {
          ...
          tanggalSelesai: editingItem.tanggalSelesai,
          vendor: editingItem.vendor,
          status: editingItem.status,
          biaya: editingItem.biaya,
          keterangan: editingItem.keterangan,
        } : null}
        title={editingItem ? 'Edit Maintenance' : 'Tambah Maintenance'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.namaAset ?? ''} - ${deleteTarget?.jenisPekerjaan ?? ''}`}
      />
    </div>
  );
};

export default GAMaintenanceSaprasDashboard;
