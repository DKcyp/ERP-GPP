...
import ServiceKendaraanModal, { ServiceKendaraanForm } from './ServiceKendaraanModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface ServiceItem extends ServiceKendaraanForm { id: string }

const seedData = (): ServiceItem[] => [
  { id: '1', platNomor: 'B 1875 ROB', merek: 'Expander', jenisService: 'Ganti Oli', tanggalService: '2025-09-15', kilometer: '45.000', biaya: '1.200.000', vendor: 'Bengkel Maju', status: 'Scheduled', keterangan: 'Oli 5W-30' },
  { id: '2', platNomor: 'B 500 GBP', merek: 'Rubicon', jenisService: 'Tune Up', tanggalService: '2025-08-20', kilometer: '60.500', biaya: '2.750.000', vendor: 'AutoPro', status: 'Done', keterangan: 'Tune up besar' },
  { id: '3', platNomor: 'B 1753 TNT', merek: 'Chery', jenisService: 'Rem', tanggalService: '2025-09-01', kilometer: '30.000', biaya: '900.000', vendor: 'Bengkel Sejahtera', status: 'Overdue', keterangan: 'Ganti kampas' },
];

const statusPill = (status: ServiceItem['status']) => {
  switch (status) {
    case 'Done':
...
const GAServiceKendaraanDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<ServiceItem[]>(seedData());
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((d) => {
      ...
      const matchesStatus = statusFilter ? d.status === (statusFilter as ServiceItem['status']) : true;

      const ds = new Date(d.tanggalService);
      const from = dateFrom ? new Date(dateFrom) : null;
      const to = dateTo ? new Date(dateTo) : null;
      const matchesDate = (!from || ds >= from) && (!to || ds <= to);

      return matchesPlat && matchesMerek && matchesStatus && matchesDate;
    });
  }, [data, searchPlat, searchMerek, statusFilter, dateFrom, dateTo]);
  ...
  const handleSave = (form: ServiceKendaraanForm) => {
    if (editingItem) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">MONITORING SERVICE KENDARAAN</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Monitoring Service Kendaraan</span>
              </nav>
            </div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Jenis Service</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Tanggal</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">KM</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 font-medium text-gray-900">{item.platNomor}</td>
                    <td className="px-2 py-1 text-gray-700">{item.merek}</td>
                    <td className="px-2 py-1 text-gray-700">{item.jenisService}</td>
                    <td className="px-2 py-1 text-gray-700">{new Date(item.tanggalService).toLocaleDateString('id-ID')}</td>
                    <td className="px-2 py-1 text-gray-700">{item.kilometer}</td>
      ...
      <ServiceKendaraanModal
        ...
        initialData={editingItem ? {
          ...
          jenisService: editingItem.jenisService,
          tanggalService: editingItem.tanggalService,
          kilometer: editingItem.kilometer,
          biaya: editingItem.biaya,
          vendor: editingItem.vendor,
          status: editingItem.status,
          keterangan: editingItem.keterangan,
        } : null}
        title={editingItem ? 'Edit Service Kendaraan' : 'Tambah Service Kendaraan'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.platNomor ?? ''} ${deleteTarget?.jenisService ?? ''}`.trim()}
      />
    </div>
  );
};

export default GAServiceKendaraanDashboard;
