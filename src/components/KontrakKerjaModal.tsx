...
const KontrakKerjaModal: React.FC<KontrakKerjaModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleInputChange = (field: keyof KontrakKerjaFormData, value: string | File | null) => {
  ...
  const handleTunjanganChange = (index: number, field: keyof TunjanganItem, value: string) => {
  ...
  const handleFileChange = (field: keyof KontrakKerjaFormData, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden animate-in zoom-in-95 fade-in-0 duration-300">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
          <h2 className="text-2xl font-bold text-gray-900">Entry Kontrak Kerja</h2>
          <button
            onClick={onClose}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('penerimaKontrak', e.target.value)}
                    className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${
                      errors.penerimaKontrak ? 'border-red-300 bg-red-50' : 'border-gray-200'
                    }`}
                    placeholder="Michael Scott"
                  />
                  {errors.penerimaKontrak && (
                    <p className="mt-1 text-sm text-red-600">{errors.penerimaKontrak}</p>
                  )}
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 gap-2 items-center">
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => handleInputChange('periodeKontrakStart', e.target.value)}
                      className={`w-full px-4 py-3 pr-12 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${
                        errors.periodeKontrakStart ? 'border-red-300 bg-red-50' : 'border-gray-200'
                      }`}
                    />
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => handleInputChange('periodeKontrakEnd', e.target.value)}
                      className={`w-full px-4 py-3 pr-12 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${
                        errors.periodeKontrakEnd ? 'border-red-300 bg-red-50' : 'border-gray-200'
                      }`}
                    />
                    <Calendar className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
                  </div>
