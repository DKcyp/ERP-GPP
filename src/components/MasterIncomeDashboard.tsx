...
const MasterIncomeDashboard: React.FC = () => {
  ...
  const handleOpenEdit = (row: IncomeEntry) => { setEditing(row); setIsModalOpen(true); };
  ...
  const handleSave = (f: PayslipFormData) => {
    if (editing) {
      ...
      const newRow: IncomeEntry = { id: newId, no: 1, kodePegawai: f.kodePegawai, namaPegawai: f.namaPegawai, jenisPegawai: f.jenisPegawai, periode: f.periode, totalIncome: f.totalIncome, totalDeduct: f.totalDeduct, gajiBersih: f.gajiBersih, keterangan: f.keterangan };
  ...
  const requestDelete = (row: IncomeEntry) => { setItemToDelete(row); setDeleteModalOpen(true); };
  const confirmDelete = () => {
