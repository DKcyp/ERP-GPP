...
import { DetailItem, BankMasukFormData } from '../types'; // Import types

interface BankMasukModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: BankMasukFormData) => void;
  initialData?: BankMasukFormData | null;
  title: string;
}
...
const BankMasukModal: React.FC<BankMasukModalProps> = ({ isOpen, onClose, onSave, initialData, title }) => {
  const today = new Date();
  const initialEmptyFormData: BankMasukFormData = {
    nomorJurnal: `BM-${new Date().getFullYear()}-${(new Date().getMonth() + 1).toString().padStart(2, '0')}-XXX`, // Dynamic default
    tanggal: today,
  ...
  const [formData, setFormData] = useState<BankMasukFormData>(initialEmptyFormData);
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      if (initialData) {
        setFormData({
          ...initialData,
          tanggal: initialData.tanggal ? new Date(initialData.tanggal) : null, // Ensure date object
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.bankAccount.trim()) {
      newErrors.bankAccount = 'Akun Bank wajib dipilih';
    }
    ...
    if (formData.detailItems.length === 0) {
      ...
      formData.detailItems.forEach((item, index) => {
        if (!item.coa.trim()) {
          newErrors[`detailItem-${index}-coa`] = 'COA wajib dipilih';
        }
        if (item.nominal <= 0) {
          newErrors[`detailItem-${index}-nominal`] = 'Nominal harus lebih dari nol';
        }
        if (!item.keterangan.trim()) {
          newErrors[`detailItem-${index}-keterangan`] = 'Keterangan detail wajib diisi';
        }
        if (!item.client.trim()) {
          newErrors[`detailItem-${index}-client`] = 'Client wajib dipilih';
        }
      });
    }
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-160px)]">
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div>
                <label htmlFor="bankAccount" className="block text-sm font-medium text-gray-700 mb-2">Bank <span className="text-red-500">*</span></label>
                <select
                  id="bankAccount"
                  className={`block w-full border rounded-lg px-4 py-2 focus:ring-blue-500 focus:border-blue-500 text-sm appearance-none ${errors.bankAccount ? 'border-red-300' : 'border-gray-300'}`}
