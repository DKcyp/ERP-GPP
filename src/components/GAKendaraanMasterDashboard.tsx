...
const GAKendaraanMasterDashboard: React.FC = () => {
  // Filters
  const [searchMerek, setSearchMerek] = useState('');
  const [searchPlat, setSearchPlat] = useState('');

  // Data
  const [data, setData] = useState<KendaraanItem[]>(seedData());
  ...
  const [deleteTarget, setDeleteTarget] = useState<KendaraanItem | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d => (
      (m ? d.merek.toLowerCase().includes(m) : true) &&
      (p ? d.platNomor.toLowerCase().includes(p) : true)
    ));
  }, [data, searchMerek, searchPlat]);
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">MASTER KENDARAAN</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="hover:text-blue-600 cursor-pointer transition-colors">Monitoring Pajak Kendaraan</span>
                <span className="mx-2">›</span>
...
export default GAKendaraanMasterDashboard;
