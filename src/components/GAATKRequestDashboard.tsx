...
import { Search, Plus, FileSpreadsheet, FileText, Clock, Edit, Trash2, User, Building } from 'lucide-react';
import ATKRequestModal, { ATKRequestForm } from './ATKRequestModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface ATKRequestItem extends ATKRequestForm { id: string }

const seedData = (): ATKRequestItem[] => [
  { id: '1', tanggal: '2025-09-05', pemohon: 'Rudi', departemen: 'Operasional', namaBarang: 'Kertas A4', kategori: 'Kertas', jumlah: '3', satuan: 'rim', status: 'Diajukan', keterangan: 'Untuk laporan bulanan' },
  { id: '2', tanggal: '2025-09-06', pemohon: 'Sari', departemen: 'HRD', namaBarang: 'Ballpoint', kategori: 'Alat Tulis', jumlah: '2', satuan: 'box', status: 'Disetujui' },
  { id: '3', tanggal: '2025-09-07', pemohon: 'Budi', departemen: 'Finance', namaBarang: 'Toner Printer', kategori: 'Printer', jumlah: '1', satuan: 'pcs', status: 'Ditolak', keterangan: 'Tidak urgent' },
];

const statusPill = (status: ATKRequestItem['status']) => {
  switch (status) {
    case 'Disetujui': return 'bg-green-100 text-green-800 border-green-200';
    case 'Diajukan': return 'bg-blue-100 text-blue-800 border-blue-200';
...
const GAATKRequestDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<ATKRequestItem[]>(seedData());
  const [animateRows, setAnimateRows] = useState(false);

  // Modals
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<ATKRequestItem | null>(null);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<ATKRequestItem | null>(null);
  ...
  const filtered = useMemo(() => {
    const qb = searchBarang.trim().toLowerCase();
    const qd = departemen.trim().toLowerCase();
    const qp = pemohon.trim().toLowerCase();
    return data.filter(d => {
      const matchBarang = qb ? `${d.namaBarang} ${d.keterangan ?? ''}`.toLowerCase().includes(qb) : true;
      const matchDep = qd ? d.departemen.toLowerCase().includes(qd) : true;
      const matchPem = qp ? d.pemohon.toLowerCase().includes(qp) : true;
      const matchStat = status ? d.status === (status as ATKRequestItem['status']) : true;
      const matchTanggal = tanggal ? d.tanggal === tanggal : true;
      return matchBarang && matchDep && matchPem && matchStat && matchTanggal;
    });
  }, [data, searchBarang, departemen, pemohon, status, tanggal]);
  ...
  const handleSave = (form: ATKRequestForm) => {
    if (editingItem) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">PENGAJUAN ATK DARI USER</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Pengajuan ATK dari User</span>
              </nav>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-6">
            ...
            <div className="space-y-2 lg:col-span-2">
              ...
              <div className="relative">
                <input type="text" value={searchBarang} onChange={(e) => setSearchBarang(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Contoh: Kertas / Toner" />
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input type="text" value={departemen} onChange={(e) => setDepartemen(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Contoh: Finance, HRD" />
            ...
            {/* Pemohon */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Pemohon</label>
              <div className="relative">
                <input type="text" value={pemohon} onChange={(e) => setPemohon(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Nama Pemohon" />
            ...
            <div className="space-y-2">
              ...
              <input type="date" value={tanggal} onChange={(e) => setTanggal(e.target.value)} className="w-full px-2 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" />
            </div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Pemohon</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Departemen</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Barang</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Kategori</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Jumlah</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'} ${animateRows ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : '0ms', animationFillMode: 'forwards' }}>
                    ...
                    <td className="px-2 py-1 text-gray-700">{item.pemohon}</td>
                    ...
                    <td className="px-2 py-1 text-gray-700">{item.jumlah}</td>
                    <td className="px-2 py-1 text-gray-700">{item.satuan}</td>
                    <td className="px-2 py-1">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${statusPill(item.status)}`}>{item.status}</span>
                    </td>
      ...
      <ATKRequestModal
        ...
        title={editingItem ? 'Edit Pengajuan ATK' : 'Tambah Pengajuan ATK'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.pemohon ?? ''} - ${deleteTarget?.namaBarang ?? ''}`}
      />
    </div>
  );
};

export default GAATKRequestDashboard;
