...
const initialData: TLD[] = [
  {
    ...
    nama: "Andi Wijaya",
    noBadge: "TLD-BA-001",
    masaBerlaku: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
  },
  {
    ...
    nama: "Budi Santoso",
    noBadge: "TLD-BA-014",
    masaBerlaku: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
  },
  {
    ...
    nama: "Citra Lestari",
    noBadge: "TLD-BA-027",
    masaBerlaku: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
  },
  {
    ...
    nama: "Dewi Kusuma",
    noBadge: "TLD-BA-042",
    masaBerlaku: new Date(Date.now() + 25 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
  },
  {
    ...
    nama: "Eko Prasetyo",
    noBadge: "TLD-BA-056",
    masaBerlaku: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
  },
  {
    ...
    nama: "Fajar Ramadhan",
    noBadge: "TLD-BA-063",
    masaBerlaku: new Date(Date.now() + 45 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
  },
];
...
const RadiographyTLDBadgeDashboard: React.FC = () => {
  ...
  const [form, setForm] = useState<Partial<TLD>>({
    nama: "",
    noBadge: "",
    masaBerlaku: "",
  });
  ...
  const [expiringAlerts, setExpiringAlerts] = useState<
    { id: string; nama: string; daysLeft: number }[]
  >([]);
  ...
  const getAlertMessage = (item: {
    ...
    if (item.daysLeft === 0)
      return `TLD Badge ${item.noBadge} (${item.nama}) kedaluwarsa hari ini!`;
    if (item.daysLeft === 1)
      return `TLD Badge ${item.noBadge} (${item.nama}) kedaluwarsa besok!`;
    if (item.daysLeft < 0)
      return `TLD Badge ${item.noBadge} (${
        item.nama
      }) sudah kedaluwarsa ${Math.abs(item.daysLeft)} hari yang lalu!`;
  ...
  const submitForm = () => {
    ...
    if (modalMode === "add") {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id === editing.id ? { ...r, nama, noBadge, masaBerlaku } : r
        )
      );
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-md p-5">
            <h2 className="text-lg font-semibold mb-4">
              {modalMode === "add" ? "Tambah Badge" : "Edit Badge"}
            </h2>
            <div className="grid grid-cols-1 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.noBadge ?? ""}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, noBadge: e.target.value }))
                  }
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
                />
              </div>
