...
interface EntrySP2DKModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: SP2DKData) => void;
  itemToEdit?: SP2DKData | null;
}

const EntrySP2DKModal: React.FC<EntrySP2DKModalProps> = ({ isOpen, onClose, onSave, itemToEdit }) => {
  const initialFormState: SP2DKData = {
    id: 0,
    namaPihak: '',
    npwp: '',
    tanggal: '',
  ...
  const [formData, setFormData] = useState<SP2DKData>(initialFormState);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({ ...prev, [name]: name === 'sp2dk' ? parseFloat(value) || 0 : value }));
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">{itemToEdit ? 'Edit SP2DK' : 'Tambah SP2DK'}</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label htmlFor="sp2dk" className="block text-sm font-medium text-gray-700 mb-1">Nilai SP2DK</label>
            <div className="relative">
              <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
              <input type="number" id="sp2dk" name="sp2dk" value={formData.sp2dk} onChange={handleChange} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" placeholder="Masukkan nilai SP2DK" min={0} required />
            </div>
          </div>
...
export default EntrySP2DKModal;
