...
interface PendebetanEntry {
  ...
  namaPegawai: string;
  jenisPegawai: string;
  tanggal: string; // ISO string e.g., 2025-09-01
  deskripsi: string;
...
const PendebetanDashboard: React.FC = () => {
  ...
  const [editing, setEditing] = useState<PendebetanEntry | null>(null);
  ...
  const [data, setData] = useState<PendebetanEntry[]>([
    {
      id: "1",
      no: 1,
      kodePegawai: "EMP001",
      ...
      jenisPegawai: "Tetap",
      tanggal: "2025-09-01",
      deskripsi: "Koreksi debet kas kecil",
      nominal: "Rp 250.000",
    },
    {
      id: "2",
      ...
      tanggal: "2025-09-03",
      deskripsi: "Debit biaya operasional",
      nominal: "Rp 500.000",
    },
  ]);

  useEffect(() => {
    // animate rows slight delay
  ...
  const handleOpenEdit = (row: PendebetanEntry) => { setEditing(row); setIsModalOpen(true); };
  ...
  const filtered = data.filter(item => {
    ...
    const startISO = toISO(tanggalAwal);
    const endISO = toISO(tanggalAkhir);
    const withinDate = (!startISO || item.tanggal >= startISO) && (!endISO || item.tanggal <= endISO);
    return matchesSearch && matchesKode && matchesNama && withinDate;
  });
  ...
  const handleSave = (f: PendebetanFormData) => {
    if (editing) {
      setData(prev => prev.map(r => r.id === editing.id ? { id: editing.id, no: r.no, kodePegawai: f.kodePegawai, namaPegawai: f.namaPegawai, jenisPegawai: f.jenisPegawai, tanggal: f.tanggal, deskripsi: f.deskripsi, nominal: f.nominal } : r));
    } else {
      const newId = (data.length + 1).toString();
      const newRow: PendebetanEntry = { id: newId, no: 1, kodePegawai: f.kodePegawai, namaPegawai: f.namaPegawai, jenisPegawai: f.jenisPegawai, tanggal: f.tanggal, deskripsi: f.deskripsi, nominal: f.nominal };
  ...
  const requestDelete = (row: PendebetanEntry) => { setItemToDelete(row); setDeleteModalOpen(true); };
  const confirmDelete = () => {
  ...
  const handleExportExcel = () => {/* TODO: wire export */};
  const handleExportPDF = () => {/* TODO: wire export */};

  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-8">Pendebetan</h1>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {pageData.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.deskripsi}</td>
...
export default PendebetanDashboard;
