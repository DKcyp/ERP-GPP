...
import { ApprovalActionData } from '../types';

interface ApprovalActionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: ApprovalActionData) => void;
...
const ApprovalActionModal: React.FC<ApprovalActionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!invoiceId || !actionType) return;

    setIsLoading(true);
    // Simulate API call
    await new Promise((resolve) => setTimeout(resolve, 1000));

    onConfirm({ invoiceId, action: actionType, keterangan });
    setIsLoading(false);
    onClose();
  };

  if (!isOpen || !invoiceId || !actionType) return null;

  const title = actionType === 'approve' ? 'Approve Invoice' : 'Reject Invoice';
