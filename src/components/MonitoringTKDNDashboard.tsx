...
const MonitoringTKDNDashboard: React.FC = () => {
  ...
  const initialData: TKDNItem[] = [
    { id: 'TKDN-001', departemen: 'finance', jenisDokumen: 'PO', nomorDokumen: 'PO-2025-0001', noSO: 'SO-10001', documentUrl: '#' },
    { id: 'TKDN-002', departemen: 'finance', jenisDokumen: 'INV', nomorDokumen: 'INV-2025-0031', noSO: 'SO-10021', documentUrl: '#' },
    { id: 'TKDN-003', departemen: 'HRD', jenisDokumen: 'Kontrak Kerja', nomorDokumen: 'KK-EMP-7788', noSO: '-', documentUrl: '#' },
  ];
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((it) =>
      (departemenFilter ? it.departemen === (departemenFilter as TKDNItem['departemen']) : true) &&
  ...
  const resetForm = () => {
    setForm({ departemen: 'finance', jenisDokumen: '', nomorDokumen: '', noSO: '', documentUrl: '#' });
  };
  ...
  const openEdit = (item: TKDNItem) => {
    setEditing(item);
    setForm({ departemen: item.departemen, jenisDokumen: item.jenisDokumen, nomorDokumen: item.nomorDokumen, noSO: item.noSO, documentUrl: item.documentUrl });
    setShowForm(true);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">MONITORING TKDN</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600">Marketing</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">Monitoring TKDN</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
            ...
            <div>
              ...
              >
                <option value="">Semua</option>
                {(departemenFilter ? jenisByDept[departemenFilter as TKDNItem['departemen']] : [...jenisByDept.finance, ...jenisByDept.HRD]).map((opt) => (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.map((it) => (
                  <tr key={it.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{it.departemen}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.jenisDokumen}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.nomorDokumen}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{it.noSO || '-'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={(e) => { if (e.currentTarget === e.target) setShowForm(false); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
              <h2 className="text-xl font-bold text-gray-900">{editing ? 'Edit TKDN' : 'Tambah TKDN'}</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setForm((f) => ({ ...f, departemen: dep, jenisDokumen: '' }));
                    }}
                    className="block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                  >
                    <option value="finance">Finance</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, jenisDokumen: e.target.value }))}
                    className="block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    value={form.noSO}
                    onChange={(e) => setForm((f) => ({ ...f, noSO: e.target.value }))}
                    className="block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
      ...
      <ConfirmDeleteModal
        ...
        itemName={deleteTarget ? `${deleteTarget.departemen} - ${deleteTarget.jenisDokumen} (${deleteTarget.nomorDokumen})` : undefined}
      />
    </div>
  );
};
