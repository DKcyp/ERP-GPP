...
interface InvoiceRow { id: number; noInvoice: string; customer: string; project: string; tanggal: string; nilai: number; status: string; jatuhTempo: string; }

const CollectionDashboardInvoice: React.FC = () => {
  ...
  const [rows] = useState<InvoiceRow[]>([
    { id: 1, noInvoice: 'INV-2025-001', customer: 'PT Sinar Abadi', project: 'Instalasi Server', tanggal: '2025-09-04', nilai: 78000000, status: 'Open', jatuhTempo: '2025-10-04' },
    { id: 2, noInvoice: 'INV-2025-002', customer: 'CV Mitra Jaya', project: 'Upgrade Network', tanggal: '2025-09-07', nilai: 52000000, status: 'Paid', jatuhTempo: '2025-10-07' },
  ]);
  ...
  ), [rows, searchInv, searchCustomer, status]);
      ...
      <div className="bg-gradient-to-r from-blue-50 to-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">COLLECTION - DASHBOARD INVOICE</h1>
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.map(r=> (
                  <tr key={r.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(r.jatuhTempo).toLocaleDateString('id-ID')}</td>
...
export default CollectionDashboardInvoice;
