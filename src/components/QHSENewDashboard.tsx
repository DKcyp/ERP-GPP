...
import { Clock, ShieldCheck, AlertTriangle, Users, CalendarCheck, Bell, X } from 'lucide-react';
...
const QHSENewDashboard: React.FC = () => {
  ...
  type AlertItem = { id: string; title: string; message: string; severity: 'warning' | 'critical'; link?: string };
  ...
  const sampleAlatUkur = [
    { id: 'au-1', kategori: 'Ultrasonic', name: 'UT-01', validUntil: '2025-11-20' }, // < 60 hari
  ];
  ...
  sampleRadiography.kamera.forEach(k => {
    if (k.activityCi > 30) {
      alerts.push({ id: k.id, title: 'Radiography - Posisi Kamera', message: `${k.name}: aktivitas ${k.activityCi} Ci (>30 Ci)`, severity: 'critical', link: '/qhse/radiography/monitoring-kamera-radiography' });
    }
  });
  sampleRadiography.sibPersonnel.forEach(p => {
    const d = daysUntil(p.validUntil);
    if (d < 90) alerts.push({ id: p.id, title: 'Radiography - SIB Personnel (90 Hari)', message: `${p.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/radiography/sib-personnel-radiasi' });
  });
  sampleRadiography.dosimeterSaku.forEach(i => {
    const d = daysUntil(i.validUntil);
    if (d < 60) alerts.push({ id: i.id, title: 'Radiography - Dosimeter Saku (60 Hari)', message: `${i.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/radiography/dosimeter-saku' });
  });
  sampleRadiography.surveymeter.forEach(i => {
    const d = daysUntil(i.validUntil);
    if (d < 60) alerts.push({ id: i.id, title: 'Radiography - Surveymeter (60 Hari)', message: `${i.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/radiography/surveymeter' });
  });
  sampleRadiography.tldBadge.forEach(i => {
    const d = daysUntil(i.validUntil);
    if (d < 30) alerts.push({ id: i.id, title: 'Radiography - TLD Badge (30 Hari)', message: `${i.name}: sisa ${d} hari`, severity: d < 15 ? 'critical' : 'warning', link: '/qhse/radiography/tld-badge' });
  });
  sampleRadiography.ujiUsap.forEach(i => {
    const d = daysUntil(i.validUntil);
    if (d < 60) alerts.push({ id: i.id, title: 'Radiography - Uji Usap (60 Hari)', message: `${i.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/radiography/uji-usap-kamera' });
  });
  sampleRadiography.kontrakSib.forEach(i => {
    const d = daysUntil(i.validUntil);
    if (d < 60) alerts.push({ id: i.id, title: 'Radiography - Kontrak SIB (60 Hari)', message: `${i.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/radiography/kontrak-sib' });
  });

  // Monitoring Daftar Alat Ukur (<60 hari)
  sampleAlatUkur.forEach(it => {
    const d = daysUntil(it.validUntil);
    if (d < 60) alerts.push({ id: it.id, title: `Alat Ukur - ${it.kategori} (60 Hari)`, message: `${it.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/monitoring-daftar-alat-ukur' });
  });

  // Monitoring Personnel
  samplePersonnel.trainingMatrix.forEach(t => {
    if (t.expired) alerts.push({ id: t.id, title: 'Personnel - Training Matrix', message: `${t.name}: certificate expired`, severity: 'critical', link: '/qhse/monitoring-personnel/training-matrix' });
  });
  samplePersonnel.mcu.forEach(p => {
    const d = daysUntil(p.validUntil);
    if (d < 60) alerts.push({ id: p.id, title: 'Personnel - MCU (60 Hari)', message: `${p.name}: sisa ${d} hari`, severity: d < 30 ? 'critical' : 'warning', link: '/qhse/monitoring-personnel/mcu' });
  });
  ...
  const [pos, setPos] = useState<{ top: number; left: number }>({ top: 80, left: 0 });
  ...
  // Drag handlers
  const dragState = useRef<{ dragging: boolean; startX: number; startY: number; origTop: number; origLeft: number }>({ dragging: false, startX: 0, startY: 0, origTop: 0, origLeft: 0 });
  ...
  const onMove = (clientX: number, clientY: number) => {
    ...
    // clamp to viewport
    nextLeft = Math.max(margin, Math.min(window.innerWidth - panelW - margin, nextLeft));
    nextTop = Math.max(margin, Math.min(window.innerHeight - panelH - margin, nextTop));
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = () => { dragState.current.dragging = false; };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div ref={panelRef} className="fixed z-50 w-96 max-w-[90vw]" style={{ top: pos.top, left: pos.left }}>
        <div className="bg-white/95 backdrop-blur rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2 text-gray-800 font-semibold">
              ...
              Alerts
              <span className="ml-2 text-xs text-gray-500">{visibleAlerts.length} items</span>
            </div>
            <button className="p-1 hover:bg-gray-200 rounded" title="Dismiss" onClick={() => { /* optional: hide panel */ }}>
              <X className="h-4 w-4 text-gray-500" />
            </button>
          </div>
          <div className="max-h-96 overflow-auto divide-y">
            {visibleAlerts.length === 0 && (
              <div className="p-4 text-sm text-gray-500">Tidak ada alert saat ini</div>
            )}
            {visibleAlerts.map(a => (
              <div key={a.id} className={`${a.severity === 'critical' ? 'bg-red-500' : 'bg-green-500'} text-white rounded-xl mx-3 my-3 p-4 shadow flex items-center justify-between`}>
                <div className="pr-3">
                  <div className="font-semibold text-sm">{a.message}</div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <a href={a.link || '#'} className="inline-flex items-center px-3 py-1.5 text-sm font-medium bg-white/90 hover:bg-white text-gray-800 rounded-md shadow">
                    Kerjakan
                  </a>
                  <button onClick={() => dismiss(a.id)} className="text-white/90 hover:text-white text-sm">
                    Close
                  </button>
                </div>
              </div>
