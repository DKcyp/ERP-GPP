...
interface Row {
  ...
  noSO: string;
  tanggalSO: string;
  statusSO: "Selesai" | "Proses";
  noDO: string;
  tanggalDO: string;
  statusDO: "Dikirim" | "Diterima" | "";
  noInvoice: string;
  tanggalInvoice: string;
  statusInvoice: "Lunas" | "Belum Dibayar" | "";
  customer: string;
  nilai: string;
  pic: string;
}
...
const mockRows: Row[] = [
  {
    ...
    no: 1,
    noSO: "SO-2025-001",
    tanggalSO: "12-01-2025",
    statusSO: "Selesai",
    noDO: "DO-2025-005",
    tanggalDO: "18-01-2025",
    statusDO: "Dikirim",
    noInvoice: "INV-2025-001",
    tanggalInvoice: "25-01-2025",
...
const ManagementMonitoringPenjualanDashboard: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    return mockRows.filter((r) => {
      const matchesNoSO = r.noSO.toLowerCase().includes(searchNoSO.toLowerCase());
      const matchesCustomer = r.customer.toLowerCase().includes(searchCustomer.toLowerCase());
      const itemDate = r.tanggalSO.split("-").reverse().join("-");
      const matchesFrom = dateFrom ? itemDate >= dateFrom : true;
      const matchesTo = dateTo ? itemDate <= dateTo : true;
      return matchesNoSO && matchesCustomer && matchesFrom && matchesTo;
    });
  }, [searchNoSO, searchCustomer, dateFrom, dateTo]);
  ...
  const renderStatus = (status: string) => {
    ...
    if (status === "Selesai" || status === "Dikirim" || status === "Diterima" || status === "Lunas") color = "bg-green-500";
    if (status === "Proses" || status === "Belum Dibayar") color = "bg-yellow-500";
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900 tracking-wide mb-1">MANAGEMENT MONITORING PENJUALAN</h1>
              <nav className="text-xs text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">Management</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">Monitoring Penjualan</span>
              </nav>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="bg-white rounded-xl shadow border border-gray-100 p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            <input type="text" value={searchNoSO} onChange={(e) => setSearchNoSO(e.target.value)} placeholder="Cari No. SO..." className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
            <input type="text" value={searchCustomer} onChange={(e) => setSearchCustomer(e.target.value)} placeholder="Cari Customer..." className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
            ...
            <table className="w-full whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">No SO</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">Tanggal SO</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">Status SO</th>
                  ...
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">No Invoice</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">Tanggal Invoice</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">Status Invoice</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900">Customer</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((r, index) => (
                  <tr key={r.id} className={`hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                    ...
                    <td className="px-3 py-2 font-medium text-gray-900">{r.noSO}</td>
                    <td className="px-3 py-2">{r.tanggalSO}</td>
                    <td className="px-3 py-2">{renderStatus(r.statusSO)}</td>
                    ...
                    <td className="px-3 py-2">{r.noInvoice}</td>
                    <td className="px-3 py-2">{r.tanggalInvoice}</td>
                    <td className="px-3 py-2">{r.statusInvoice ? renderStatus(r.statusInvoice) : ''}</td>
...
export default ManagementMonitoringPenjualanDashboard;
