...
import { ApprovalPenggajianActionData } from '../types';

interface ApprovalPenggajianActionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: ApprovalPenggajianActionData) => void;
  penggajianId: string | null;
  actionType: 'approve' | 'reject' | null;
}

const ApprovalPenggajianActionModal: React.FC<ApprovalPenggajianActionModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  penggajianId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!penggajianId || !actionType) return;

    setIsLoading(true);
    // Simulate API call
    await new Promise((resolve) => setTimeout(resolve, 1000));

    onConfirm({ penggajianId, action: actionType, keterangan });
    setIsLoading(false);
    onClose();
  };

  if (!isOpen || !penggajianId || !actionType) return null;

  const title = actionType === 'approve' ? 'Approve Penggajian' : 'Reject Penggajian';
...
export default ApprovalPenggajianActionModal;
