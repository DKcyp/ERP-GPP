...
const KontrakDealDashboard: React.FC = () => {
  ...
  const [jenisKontrakDropdownOpen, setJenisKontrakDropdownOpen] =
    useState(false);
  const [statusKontrakDropdownOpen, setStatusKontrakDropdownOpen] =
    useState(false);
  ...
  const [editingKontrakDeal, setEditingKontrakDeal] =
    useState<KontrakDealFormData | null>(null);
  ...
  const handleSaveKontrakDeal = (formData: KontrakDealFormData) => {
    if (formData.id) {
      // If formData has an ID, it's an edit operation
      setKontrakDeals((prev) =>
        prev.map((kontrak) =>
      ...
      const newKontrakDeal: KontrakDeal = {
        ...
        namaKontrak: formData.namaKontrak,
        jenisKontrak: formData.jenisKontrak,
        tanggalKontrak: new Date(formData.tanggalKontrak).toLocaleDateString(
          "id-ID",
          {
            day: "2-digit",
            month: "2-digit",
  ...
  const filteredKontrakDeals = kontrakDeals.filter((kontrak) => {
    const matchesNoKontrak = kontrak.id
      .toLowerCase()
      .includes(searchNoKontrak.toLowerCase());
    const matchesPIC = kontrak.pic
      .toLowerCase()
      .includes(searchPIC.toLowerCase());
    const matchesJenisKontrak = selectedJenisKontrak
      ? kontrak.jenisKontrak === selectedJenisKontrak
      : true;
    const matchesLokasiPekerjaan = kontrak.lokasiPekerjaan
      .toLowerCase()
      .includes(searchLokasiPekerjaan.toLowerCase());
    const matchesStatusKontrak = selectedStatusKontrak
      ? kontrak.statusPenawaran === selectedStatusKontrak
      : true;

    // Date filtering logic
    const kontrakDate = new Date(
      kontrak.tanggalKontrak.split("-").reverse().join("-")
    ); // Convert DD-MM-YYYY to YYYY-MM-DD
    const fromDate = dateFrom ? new Date(dateFrom) : null;
    const toDate = dateTo ? new Date(dateTo) : null;

    const matchesDate =
      (!fromDate || kontrakDate >= fromDate) &&
      (!toDate || kontrakDate <= toDate);

    return (
      matchesNoKontrak &&
      matchesPIC &&
      matchesJenisKontrak &&
      matchesLokasiPekerjaan &&
      matchesStatusKontrak &&
      matchesDate
    );
  });
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <button
                    onClick={() =>
                      setJenisKontrakDropdownOpen(!jenisKontrakDropdownOpen)
                    }
                  ...
                  >
                    <span
                      className={
                        selectedJenisKontrak ? "text-gray-900" : "text-gray-500"
                      }
                    >
                    ...
                    <ChevronDown
                      className={`h-3 w-3 text-gray-400 transition-transform duration-200 ${
                        jenisKontrakDropdownOpen ? "rotate-180" : ""
                      }`}
                    />
                  </button>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <button
                    onClick={() =>
                      setStatusKontrakDropdownOpen(!statusKontrakDropdownOpen)
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 flex items-center justify-between bg-white text-xs"
                  >
                    <span
                    ...
                    <ChevronDown
                      className={`h-3 w-3 text-gray-400 transition-transform duration-200 ${
                        statusKontrakDropdownOpen ? "rotate-180" : ""
                      }`}
                    />
                  </button>
                ...
                <tbody className="divide-y divide-gray-200">
                  {currentData.map((kontrak, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-gray-600 text-xs">
                        {kontrak.jenisKontrak}
                      </td>
                      <td className="px-3 py-2 text-gray-600 text-xs">
                        {kontrak.tanggalKontrak}
                      </td>
                      <td className="px-3 py-2 text-gray-600 text-xs">
                        {kontrak.durasiKontrak}
                      </td>
                      <td className="px-3 py-2 text-gray-600 font-medium text-xs">
                        {kontrak.nilaiKontrak}
                      </td>
                      <td className="px-3 py-2 text-gray-600 text-xs">
                        {kontrak.lokasiPekerjaan}
                      </td>
                      <td className="px-3 py-2">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium border ${getStatusPenawaranColor(
            ...
            <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 text-xs">
              <div className="flex items-center justify-between">
                <div className="text-xs text-gray-700">
                  ...
                  {Math.min(endIndex, filteredKontrakDeals.length)} of{" "}
                  {filteredKontrakDeals.length} results
                </div>
                <div className="flex items-center space-x-2">
                  <button
        ...
        <KontrakDealModal
          ...
          title={
            editingKontrakDeal ? "Edit Kontrak Deal" : "Entry Kontrak Deal"
          } // Dynamic title
        />
