...
import { SP2DKData } from '../types';
import EntrySP2DKModal from './EntrySP2DKModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

const dummyData: SP2DKData[] = [
  { id: 1, namaPihak: 'PT Samudera Kencana', npwp: '22.333.444.5-666.000', tanggal: '2024-09-30', sp2dk: 17500000 },
];

const SP2DKDashboard: React.FC = () => {
  const [data, setData] = useState<SP2DKData[]>(dummyData);
  const [searchTerm, setSearchTerm] = useState('');

  const [isAddEditModalOpen, setIsAddEditModalOpen] = useState(false);
  const [itemToEdit, setItemToEdit] = useState<SP2DKData | null>(null);
  ...
  const handleEditClick = (item: SP2DKData) => { setItemToEdit(item); setIsAddEditModalOpen(true); };
  const handleSaveItem = (newItem: SP2DKData) => {
    if (newItem.id) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">SP2DK</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">Tax</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">SP2DK</span>
              </nav>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Nilai SP2DK</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">{formatCurrency(item.sp2dk)}</td>
                ...
                {filteredData.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data SP2DK yang ditemukan.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <EntrySP2DKModal isOpen={isAddEditModalOpen} onClose={handleCloseAddEditModal} onSave={handleSaveItem} itemToEdit={itemToEdit} />
