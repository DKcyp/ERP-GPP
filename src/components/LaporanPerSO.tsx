...
import { Search, Printer, FileText, Calendar, ChevronDown, ChevronUp, FileDown } from 'lucide-react';

interface LaporanPerSOProps {
  // Add any props here if needed
}

type StatusType = 'Draft' | 'Approved' | 'In Progress' | 'Completed' | 'Cancelled' | 'On Hold' | 'Selesai' | 'On Progress';
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-800">LAPORAN PER SALES ORDER</h1>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md mb-6">
          <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
            ...
            <div>
              <label htmlFor="noSo" className="block text-sm font-medium text-gray-700 mb-1">Nomor SO</label>
            ...
            {/* Nama Customer */}
            <div>
              <label htmlFor="customer" className="block text-sm font-medium text-gray-700 mb-1">Nama Customer</label>
              <div className="relative">
                <input
                  type="text"
                  id="customer"
            ...
            {/* Status SO */}
            <div>
              <label htmlFor="status" className="block text-sm font-medium text-gray-700 mb-1">Status SO</label>
              <select
                id="status"
                value={statusFilter}
            ...
            <table className="w-full text-sm text-left text-gray-600">
              <thead className="bg-gray-100 text-gray-700 uppercase text-xs">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3">No. SO</th>
                  <th scope="col" className="px-6 py-3">Customer</th>
                  <th scope="col" className="px-6 py-3">Project</th>
                  <th scope="col" className="px-6 py-3 text-right">Nilai Kontrak</th>
                ...
                {salesOrderData.map((item) => (
                  <React.Fragment key={item.id}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">{formatCurrency(item.nilaiKontrak)}</td>
                      <td className="px-6 py-4 text-right">{formatCurrency(item.revenue)}</td>
                      <td className="px-6 py-4 text-right">{formatCurrency(item.biaya)}</td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 text-xs font-semibold rounded-full ${getStatusBadgeColor(item.status)}`}>
                          {item.status}
                        </span>
                      </td>
                    </tr>
                      ...
                      <tr className="bg-gray-50">
                        <td colSpan={9} className="px-6 py-4">
                          <div className="pl-6 border-l-4 border-blue-400">
                            <h4 className="font-semibold text-gray-800 mb-3">Detail Pekerjaan</h4>
                            <div className="overflow-x-auto">
                                ...
                                <tbody>
                                  {item.details?.map((detail, idx) => (
                                    <tr key={idx} className="border-t border-gray-200">
                                      <td className="px-4 py-3">{detail.pekerjaan}</td>
                                      <td className="px-4 py-3 text-right">{formatCurrency(detail.biaya)}</td>
                                      <td className="px-4 py-3">
                                        <div className="w-full bg-gray-200 rounded-full h-2.5">
                                          <div 
                                            className={`h-2.5 rounded-full ${detail.progress === 100 ? 'bg-green-600' : 'bg-blue-600'}`} 
                                            style={{ width: `${detail.progress}%` }}
                                          ></div>
                                        </div>
                                        <span className="text-sm text-gray-600">{detail.progress}%</span>
                                      </td>
                                      <td className="px-4 py-3">
                                        <div className="flex flex-wrap gap-1">
                                          {detail.lampiran.length > 0 ? (
                                            detail.lampiran.map((file, fileIdx) => (
                                              <span 
                                                key={fileIdx}
                            ...
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="bg-blue-50 p-4 rounded-lg">
                                <h5 className="text-sm font-medium text-blue-800">Total Nilai Kontrak</h5>
                                <p className="text-xl font-bold text-blue-600">{formatCurrency(item.nilaiKontrak)}</p>
                              </div>
                              <div className="bg-green-50 p-4 rounded-lg">
                                <h5 className="text-sm font-medium text-green-800">Total Revenue</h5>
                                <p className="text-xl font-bold text-green-600">{formatCurrency(item.revenue)}</p>
                              </div>
                              <div className="bg-purple-50 p-4 rounded-lg">
                                <h5 className="text-sm font-medium text-purple-800">Total Biaya</h5>
                                <p className="text-xl font-bold text-purple-600">{formatCurrency(item.biaya)}</p>
                              </div>
                            </div>
                            ...
                            <div className="mt-4 p-4 bg-gray-100 rounded-lg">
                              <div className="flex items-center justify-between">
                                ...
                                <span className={`text-xl font-bold ${item.margin >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {formatCurrency(item.margin)} ({((item.margin / item.revenue) * 100).toFixed(2)}%)
                                </span>
                              </div>
                            </div>
                          </div>
