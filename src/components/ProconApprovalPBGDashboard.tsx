...
interface PermintaanRecord {
  id: string; // kodePermintaan
  kodePermintaan: string;
  tanggalInput: string; // yyyy-MM-dd
  gudangAsal: string; // id
  gudangTujuan: string; // id
  status: Status;
  items: RequestItem[];
}
...
const ProconApprovalPBGDashboard: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<'all' | 'pending' | 'approved' | 'rejected'>('all');
  const [detailOpen, setDetailOpen] = useState(false);
  const [selected, setSelected] = useState<PermintaanRecord | null>(null);

  const [data, setData] = useState<PermintaanRecord[]>([
    {
      id: 'PRG-20250719-001',
    ...
    {
      id: 'PRG-20250718-002',
      kodePermintaan: 'PRG-20250718-002',
      tanggalInput: '2025-07-18',
      gudangAsal: 'G002',
      gudangTujuan: 'G003',
      status: 'Approved',
  ...
  const filtered = useMemo(() => {
    return data.filter(r => {
      ...
      const matchesSearch = r.kodePermintaan.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const openDetail = (rec: PermintaanRecord) => {
    setSelected(rec);
    setDetailOpen(true);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <nav className="text-sm text-gray-600">
                ...
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">PBG</span>
                <span className="mx-2">›</span>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Action Bar (filters only; NO Add button) */}
        ...
        {/* Table mirrors PermintaanBarangGudangDashboard */}
              ...
              {filtered.length > 0 ? (
                filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{r.kodePermintaan}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{r.tanggalInput}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        <button
                          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border text-gray-700 hover:bg-gray-50 text-xs"
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-2xl font-bold text-gray-900">Detail Permintaan - {selected.kodePermintaan}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4 text-sm">
                <div>
                  ...
                  <div className="font-medium text-gray-900">{selected.tanggalInput}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-medium">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${badgeClass(selected.status)}`}>{selected.status}</span>
                  </div>
                </div>
              </div>
              {/* Items table */}
...
export default ProconApprovalPBGDashboard;
