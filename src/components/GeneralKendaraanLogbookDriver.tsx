...
import { Plus, Edit, Trash2, Search, FileSpreadsheet, FileText, Clock, X, Save } from "lucide-react";

type Status = "Pengajuan" | "Proses" | "Selesai";
interface LogbookDriverItem {
  id: string;
...
const seed: LogbookDriverItem[] = [
  { id: crypto.randomUUID(), tanggal: "2025-09-01", namaDriver: "Budi", kendaraan: "Mobil A", kmAwal: 1000, kmAkhir: 1050, tujuan: "Customer A", status: "Proses" },
];

const GeneralKendaraanLogbookDriver: React.FC = () => {
  const [rows, setRows] = useState<LogbookDriverItem[]>(seed);
  const [q, setQ] = useState("");
  const [statusFilter, setStatusFilter] = useState<"" | Status>("");
  const [isOpen, setIsOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<LogbookDriverItem>({ id: "", tanggal: "", namaDriver: "", kendaraan: "", kmAwal: 0, kmAkhir: 0, tujuan: "", status: "Pengajuan" });
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => (
      ...
      (r.namaDriver.toLowerCase().includes(qq) || r.kendaraan.toLowerCase().includes(qq) || r.tujuan.toLowerCase().includes(qq))
    ));
  }, [rows, q, statusFilter]);

  const openAdd = () => {
    setEditingId(null);
    setForm({ id: crypto.randomUUID(), tanggal: "", namaDriver: "", kendaraan: "", kmAwal: 0, kmAkhir: 0, tujuan: "", status: "Pengajuan" });
    setIsOpen(true);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">General - Kendaraan - Logbook Driver</h1>
            <div className="text-xs text-gray-600">General › Kendaraan › Logbook Driver</div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow border border-gray-100 p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Cari Driver/Kendaraan/Tujuan" className="w-72 pl-8 pr-2 py-2 text-xs border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
          ...
          <table className="w-full text-xs">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-3 py-2 text-left">KM Awal</th>
                <th className="px-3 py-2 text-left">KM Akhir</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((r, idx) => (
                <tr key={r.id} className={idx % 2 === 0 ? "bg-white" : "bg-gray-25"}>
                  ...
                  <td className="px-3 py-2">{r.kmAwal.toLocaleString("id-ID")}</td>
                  <td className="px-3 py-2">{r.kmAkhir.toLocaleString("id-ID")}</td>
              ...
              {filtered.length===0 && (
                <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-500">Tidak ada data</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={(e)=> e.target===e.currentTarget && setIsOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
              <h2 className="text-lg font-semibold">{editingId? 'Edit':'Tambah'} Logbook</h2>
            ...
            <form onSubmit={saveForm} className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">KM Awal</label>
                  <input type="number" value={form.kmAwal} onChange={e=>setForm(f=>({...f, kmAwal:Number(e.target.value||0)}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" min={0} required/>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">KM Akhir</label>
                  <input type="number" value={form.kmAkhir} onChange={e=>setForm(f=>({...f, kmAkhir:Number(e.target.value||0)}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" min={0} required/>
                </div>
...
export default GeneralKendaraanLogbookDriver;
