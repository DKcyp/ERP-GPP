...
const hutangUsahaData: HutangUsahaItem[] = [
  { no: 1, namaVendor: 'Bintang Selular', totalHutang: '100.000', status: 'Belum Lunas' },
  { no: 2, namaVendor: 'ACE GROUP', totalHutang: '150.000', status: 'Belum Lunas' },
  { no: 3, namaVendor: 'ARS PT', totalHutang: '500.000', status: 'Lunas' },
  { no: 4, namaVendor: 'Cakra Nusantara', totalHutang: '350.000', status: 'Lunas' },
  { no: 5, namaVendor: 'Action Wisdom', totalHutang: '200.000', status: 'Lunas' },
];

const LaporanHutangUsahaDashboard: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [entriesPerPage, setEntriesPerPage] = useState(10);

  const totalPages = Math.ceil(hutangUsahaData.length / entriesPerPage);
  const currentEntries = hutangUsahaData.slice(
    (currentPage - 1) * entriesPerPage,
    currentPage * entriesPerPage
  );

  const getStatusClasses = (status: HutangUsahaItem['status']) => {
    switch (status) {
      case 'Belum Lunas':
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <nav className="text-sm text-gray-600">
                ...
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-medium">Laporan Hutang Usaha</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentEntries.map((item) => (
                  <tr key={item.no} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.totalHutang}</td>
          ...
          >
            <div className="text-sm text-gray-700">
              Showing {((currentPage - 1) * entriesPerPage) + 1} to {Math.min(currentPage * entriesPerPage, hutangUsahaData.length)} of {hutangUsahaData.length} entries
            </div>
...
export default LaporanHutangUsahaDashboard;
