...
interface PIRow { id: number; noPI: string; customer: string; project: string; tanggal: string; nilai: number; status: string; }

const CollectionDashboardProformaInvoice: React.FC = () => {
  const today = new Date();
  const [searchPI, setSearchPI] = useState('');
  const [searchCustomer, setSearchCustomer] = useState('');
  const [status, setStatus] = useState('');

  const [rows] = useState<PIRow[]>([
    { id: 1, noPI: 'PI-2025-001', customer: 'PT Sinar Abadi', project: 'Instalasi Server', tanggal: '2025-09-03', nilai: 45000000, status: 'Open' },
    { id: 2, noPI: 'PI-2025-002', customer: 'CV Mitra Jaya', project: 'Upgrade Network', tanggal: '2025-09-06', nilai: 32000000, status: 'Closed' },
  ]);

  const filtered = useMemo(() => rows.filter(r =>
    (searchPI ? r.noPI.toLowerCase().includes(searchPI.toLowerCase()) : true) &&
    (searchCustomer ? r.customer.toLowerCase().includes(searchCustomer.toLowerCase()) : true) &&
    (status ? r.status === status : true)
  ), [rows, searchPI, searchCustomer, status]);
      ...
      <div className="bg-gradient-to-r from-blue-50 to-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">COLLECTION - DASHBOARD PROFORMA INVOICE</h1>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">No PI</label>
              <input className="block w-full border border-gray-300 rounded-lg px-4 py-2 text-sm" placeholder="PI-..." value={searchPI} onChange={e=>setSearchPI(e.target.value)} />
            </div>
            <div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.map(r=> (
                  <tr key={r.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600">{r.noPI}</td>
...
export default CollectionDashboardProformaInvoice;
