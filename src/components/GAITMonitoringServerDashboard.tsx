...
import ServerModal, { ServerForm } from './ServerModal';
import ConfirmDeleteModal from './ConfirmDeleteModal';

interface ServerItem extends ServerForm { id: string }

const seedData = (): ServerItem[] => [
  { id: '1', serverName: 'SRV-APP-01', ipAddress: '192.168.1.10', location: 'Data Center', os: 'Ubuntu 22.04', role: 'Application', uptime: '21 days', cpu: '32%', memory: '12/32 GB', storage: '350/1000 GB', status: 'Online', lastChecked: '2025-09-07', notes: 'Normal' },
  { id: '2', serverName: 'SRV-DB-01', ipAddress: '192.168.1.20', location: 'Data Center', os: 'Ubuntu 22.04', role: 'Database', uptime: '7 days', cpu: '75%', memory: '28/32 GB', storage: '700/1000 GB', status: 'Degraded', lastChecked: '2025-09-08', notes: 'CPU tinggi' },
  { id: '3', serverName: 'SRV-BKP-01', ipAddress: '192.168.1.30', location: 'Kantor', os: 'Windows Server 2019', role: 'Backup', uptime: '2 days', cpu: '10%', memory: '4/16 GB', storage: '100/2000 GB', status: 'Maintenance', lastChecked: '2025-09-06', notes: 'Perawatan' },
];
...
const GAITMonitoringServerDashboard: React.FC = () => {
  // Filters
  ...
  // Data
  const [data, setData] = useState<ServerItem[]>(seedData());
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d => {
      const matchText = q ? `${d.serverName} ${d.ipAddress} ${d.os} ${d.notes ?? ''}`.toLowerCase().includes(q) : true;
      const matchStatus = status ? d.status === (status as ServerItem['status']) : true;
      const matchRole = role ? d.role === (role as ServerItem['role']) : true;
      const matchLoc = loc ? d.location.toLowerCase().includes(loc) : true;

      const dt = new Date(d.lastChecked);
      const from = dateFrom ? new Date(dateFrom) : null;
      const to = dateTo ? new Date(dateTo) : null;
      const matchDate = (!from || dt >= from) && (!to || dt <= to);

      return matchText && matchStatus && matchRole && matchLoc && matchDate;
    });
  }, [data, search, status, role, location, dateFrom, dateTo]);
  ...
  const handleSave = (form: ServerForm) => {
    if (editingItem) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">MONITORING SERVER</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">Monitoring Server</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6 mb-6">
            ...
            <div className="space-y-2 lg:col-span-2">
              <label className="block text-xs font-medium text-gray-700">Cari (Nama/IP/OS/Keterangan)</label>
              <div className="relative">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="SRV-APP-01 / 192.168.1.10 / Ubuntu" />
            ...
            <div className="space-y-2">
              ...
              <select value={role} onChange={(e) => setRole(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs bg-white">
                <option value="">Semua</option>
            ...
            <div className="space-y-2">
              ...
              <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} className="w-full px-2 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-xs" placeholder="Data Center / Kantor" />
            </div>

            {/* Range tanggal */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Dicek Dari</label>
              <div className="relative">
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Nama Server</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">IP</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Lokasi</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">OS</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Role</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Uptime</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">CPU</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Memory</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Storage</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Status</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Last Checked</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'} ${animateRows ? 'animate-in fade-in slide-in-from-bottom-2' : 'opacity-0'}`} style={{ animationDelay: animateRows ? `${index * 100}ms` : '0ms', animationFillMode: 'forwards' }}>
                    ...
                    <td className="px-2 py-1 font-medium text-gray-900">{item.serverName}</td>
                    <td className="px-2 py-1 text-gray-700">{item.ipAddress}</td>
                    <td className="px-2 py-1 text-gray-700">{item.location}</td>
                    <td className="px-2 py-1 text-gray-700">{item.os}</td>
                    <td className="px-2 py-1 text-gray-700">{item.role}</td>
                    <td className="px-2 py-1 text-gray-700">{item.uptime}</td>
                    <td className="px-2 py-1 text-gray-700">{item.cpu}</td>
                    <td className="px-2 py-1 text-gray-700">{item.memory}</td>
                    <td className="px-2 py-1 text-gray-700">{item.storage}</td>
                    ...
                    <td className="px-2 py-1 text-gray-700">{new Date(item.lastChecked).toLocaleDateString('id-ID')}</td>
      ...
      <ServerModal
        ...
        initialData={editingItem ? {
          ...
          uptime: editingItem.uptime,
          cpu: editingItem.cpu,
          memory: editingItem.memory,
          storage: editingItem.storage,
          status: editingItem.status,
          lastChecked: editingItem.lastChecked,
          notes: editingItem.notes,
        } : null}
        title={editingItem ? 'Edit Server' : 'Tambah Server'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={`${deleteTarget?.serverName ?? ''} - ${deleteTarget?.ipAddress ?? ''}`}
      />
    </div>
  );
};

export default GAITMonitoringServerDashboard;
