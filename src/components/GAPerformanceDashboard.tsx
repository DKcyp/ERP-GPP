...
interface GAPerformanceItem extends GAPerformanceForm { id: string }

const seedData = (): GAPerformanceItem[] => [
  { id: '1', indikator: 'SLA Penanganan Tiket', target: '95%', realisasi: '92%', periode: '2025-08', kategori: 'IT', keterangan: 'Kendala vendor' },
  { id: '2', indikator: 'Kepatuhan Legalitas', target: '100%', realisasi: '98%', periode: '2025-08', kategori: 'Legalitas' },
  { id: '3', indikator: 'Pemeliharaan Sarana', target: '100%', realisasi: '96%', periode: '2025-08', kategori: 'Sarana & Prasarana' },
];

const GAPerformanceDashboard: React.FC = () => {
  // Filters
  ...
  // Data state
  const [data, setData] = useState<GAPerformanceItem[]>(seedData());
  const [animateRows, setAnimateRows] = useState(false);

  // Modals
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<GAPerformanceItem | null>(null);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<GAPerformanceItem | null>(null);
  ...
  const filtered = useMemo(() => {
    const q = searchIndikator.trim().toLowerCase();
    return data.filter(d => {
      const matchInd = q ? d.indikator.toLowerCase().includes(q) : true;
      const matchPer = periode ? d.periode === periode : true;
      const matchKat = kategori ? d.kategori === (kategori as GAPerformanceItem['kategori']) : true;
      return matchInd && matchPer && matchKat;
    });
  }, [data, searchIndikator, periode, kategori]);
  ...
  const handleSave = (form: GAPerformanceForm) => {
    if (editingItem) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-white border-b border-blue-100">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-wide mb-2">GA PERFORMANCE</h1>
              <nav className="text-sm text-gray-600">
                <span className="hover:text-blue-600 cursor-pointer transition-colors">GA</span>
                <span className="mx-2">›</span>
                <span className="text-blue-600 font-semibold">GA Performance</span>
              </nav>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Cari Indikator</label>
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                <tr>
                  ...
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Indikator</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Kategori</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Target</th>
                  <th className="px-2 py-1 text-left text-xs font-semibold text-gray-900">Realisasi</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {currentData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 font-medium text-gray-900">{item.indikator}</td>
                    <td className="px-2 py-1 text-gray-700">{item.kategori}</td>
                    <td className="px-2 py-1 text-gray-700">{item.target}</td>
                    <td className="px-2 py-1 text-gray-700">{item.realisasi}</td>
      ...
      <GAPerformanceModal
        ...
        title={editingItem ? 'Edit GA Performance' : 'Tambah GA Performance'}
      ...
      <ConfirmDeleteModal
        ...
        itemName={deleteTarget?.indikator}
      />
    </div>
  );
};

export default GAPerformanceDashboard;
