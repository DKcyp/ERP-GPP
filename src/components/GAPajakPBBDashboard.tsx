...
const GAPajakPBBDashboard: React.FC = () => {
  const [items, setItems] = useState<PBBItem[]>([
    { id: crypto.randomUUID(), namaObjek: "Kantor Pusat", nop: "32.12.345.678.910-111", nilai: 15000000, jatuhTempo: "2025-12-31", status: "Belum Lunas" },
  ]);
  const [q, setQ] = useState("");
  const [isOpen, setIsOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<PBBItem>({ id: "", namaObjek: "", nop: "", nilai: 0, jatuhTempo: "", status: "Belum Lunas" });
  const [saving, setSaving] = useState(false);

  const filtered = useMemo(() => {
    const qq = q.toLowerCase();
    return items.filter((it) => it.namaObjek.toLowerCase().includes(qq) || it.nop.toLowerCase().includes(qq));
  }, [items, q]);

  const openAdd = () => {
    setEditingId(null);
    setForm({ id: crypto.randomUUID(), namaObjek: "", nop: "", nilai: 0, jatuhTempo: "", status: "Belum Lunas" });
    setIsOpen(true);
  };
  ...
  const handleDelete = (id: string) => {
    if (!confirm("Hapus entri PBB ini?")) return;
    setItems((prev) => prev.filter((x) => x.id !== id));
  };

  const exportCSV = () => {
    const header = ["Nama Objek", "NOP", "Nilai", "Jatuh Tempo", "Status"]; 
    const rows = filtered.map((r) => [r.namaObjek, r.nop, r.nilai, r.jatuhTempo, r.status]);
    ...
    a.href = url;
    a.download = `PBB_${new Date().toISOString().slice(0,10)}.csv`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Monitoring Pajak GA - PBB</h1>
            <div className="text-xs text-gray-600">GA › Monitoring Pajak › PBB</div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow border border-gray-100 p-4 flex items-center justify-between">
          ...
          <div className="relative">
            <input
              ...
              placeholder="Cari Nama Objek / NOP"
          ...
          <table className="w-full text-xs">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-3 py-2 text-left">Nama Objek</th>
                <th className="px-3 py-2 text-left">NOP</th>
                <th className="px-3 py-2 text-right">Nilai PBB</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((r, idx) => (
                <tr key={r.id} className={idx % 2 === 0 ? "bg-white" : "bg-gray-25"}>
                  ...
                  <td className="px-3 py-2 font-medium text-gray-900">{r.namaObjek}</td>
                  <td className="px-3 py-2 text-gray-800">{r.nop}</td>
                  <td className="px-3 py-2 text-gray-800 text-right">{currency(r.nilai)}</td>
                  <td className="px-3 py-2 text-gray-800">{new Date(r.jatuhTempo).toLocaleDateString("id-ID")}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={(e) => e.target === e.currentTarget && setIsOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
              <h2 className="text-lg font-semibold">{editingId ? "Edit" : "Tambah"} PBB</h2>
            ...
            <form onSubmit={saveForm} className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Nama Objek</label>
                  <input
                    value={form.namaObjek}
                    onChange={(e) => setForm((f) => ({ ...f, namaObjek: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                    required
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">NOP</label>
                  <input
                    value={form.nop}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Nilai PBB</label>
                  <input
                    type="number"
                    value={form.nilai}
                    onChange={(e) => setForm((f) => ({ ...f, nilai: Number(e.target.value || 0) }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                    min={0}
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, jatuhTempo: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                    required
                  />
                </div>
                <div>
                  ...
                  <select
                    value={form.status}
                    onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as PBBItem["status"] }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                  >
...
export default GAPajakPBBDashboard;
